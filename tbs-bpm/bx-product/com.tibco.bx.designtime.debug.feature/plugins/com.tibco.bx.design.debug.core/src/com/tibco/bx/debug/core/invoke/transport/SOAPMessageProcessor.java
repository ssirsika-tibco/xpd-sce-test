...
import com.tibco.bx.debug.core.invoke.datamodel.LaunchWsdlBindingElement;
import com.tibco.bx.debug.core.invoke.datamodel.LaunchWsdlOperationElement;
import com.tibco.bx.debug.core.ws.util.SoapHelper;
...
public class SOAPMessageProcessor  implements ISerializer, IDeserializer {
	...
	private static final String DEFAULT_SOAP_ENCODING = "UTF-8";//$NON-NLS-1$	
	private static final String LINE_SEPARATOR = System.getProperties().getProperty("line.separator");//$NON-NLS-1$
	 ...
	 */
	private String[] getEncodingInfo(LaunchWsdlOperationElement context) {
		
		String[] info = new String[] { null, null };
		
//		Iterator iter = context.getBindingElement().getBindingOperation().getBindingInput()
//			.getExtensibilityElements().iterator();
		LaunchWsdlBindingElement bindingElement = context.getBindingElement();
		BindingOperation bo = bindingElement.getBindingOperation();
		BindingInput bi = bo.getEBindingInput();
		List list = bi.getExtensibilityElements();
		Iterator iter = list.iterator();
     ...
     * qualified.
     */
	private Element createRPCWrapper(Document document, LaunchWsdlOperationElement context, Hashtable namespaceTable) {
	...
	void initMessage(ISOAPMessage message) {
		...
		try {
			...
			if (!message.getOperation().isDocumentStyle()) {
				Element rpcWrapper = createRPCWrapper(document, message.getOperation(), namespaceTable);
...
//			throw new TransportException(e);
	...
	private Element getSOAPElement(Element root, String localName, Map namespaceTable) {
		...
		NodeList list = root.getElementsByTagName(prefix + ":" + localName);//$NON-NLS-1$
		if (list.getLength() == 0)
			return null;
	...
	private void addEncodingStyle(Element[] elements, String encodingStyle) {
		...
		for (int i=0; i < elements.length; i++) {
	    	...
	    	elements[i].setAttribute(
	    			Constants.NS_PREFIX_SOAP_ENV + ":" + Constants.ATTR_ENCODING_STYLE, encodingStyle);	//$NON-NLS-1$		
		}
	}
	...
	private String serializeElements(Element[] elements) {
		...
		for (int i = 0; i < elements.length; i++) {
			...
			if (serializedFragment == null) {
				...
				elements[i].setAttribute("xmlns:xsi", Constants.URI_2001_SCHEMA_XSI);//$NON-NLS-1$
	...
	private boolean isRPCRequest(ISOAPMessage message) {
		return 
			!message.getOperation().isDocumentStyle() &&									// rpc style
			...
			case ISOAPMessage.ENVELOPE:
				...
				if (isRPCRequest(message)) {					
					...
					if (body != null) {
						...
						addEncodingStyle(bodyContent, getEncodingInfo(message.getOperation())[1]);
					}
				}
	 ...
	 */
	private Element[] fixSOAPResponse(NodeList instanceList, LaunchWsdlOperationElement context) {
		
		Vector instanceVector = toElementVector(instanceList);		
		Element[] instanceDocuments = new Element[instanceVector.size()];
		
		Operation oper = context.getOperation();		
	...
	private void processBody(Element envelope, ISOAPMessage message) {
	    ...
	    if (message.getOperation().isDocumentStyle())
	        ...
	        NodeList rpcWrapper = body.getElementsByTagNameNS("*",  //$NON-NLS-1$
	        		message.getOperation().getOperation().getOutput().getMessage().getQName().getLocalPart());
	        ...
	        */
	        if (rpcWrapper.getLength() <= 0)
	        	rpcWrapper = body.getElementsByTagNameNS("*", "*");//$NON-NLS-1$ //$NON-NLS-2$
	    ...
	    message.setBodyContent(fixSOAPResponse(instanceList, message.getOperation()));
	}
	...
	private String addRootElement(String xml, Map namespaceTable) {
	    ...
	    while (iter.hasNext()) {
	    	...
	    	sb.append(" ").append("xmlns:").append(prefix).append("=\"").append(uri).append("\""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
	    }	    
	    
	    sb.append(">").append(xml).append("</root>"); //$NON-NLS-1$ //$NON-NLS-2$
	    return sb.toString();
	}
