 ...
 * @generated
 */
public class BadgeLabelModelIconEditPart extends CompartmentEditPart implements
        ITextAwareEditPart {
     ...
     * @generated
     */
    public BadgeLabelModelIconEditPart(View view) {
        super(view);
    }
    ...
    protected Image getLabelIcon() {
        ...
        Object viewMod = getModel();

        if (viewMod instanceof View) {
            View view = (View) viewMod;

            Diagram diagram = view.getDiagram();

            if (diagram != null) {
                if (BomUIUtil.isUserDiagram(diagram)) {
                    return com.tibco.xpd.bom.resources.ui.Activator
                            .getDefault().getImageRegistry()
                            .get(BOMImages.BADGEMODEL_SUBDIAG);
                }
            }
        }
        ...
        if (elem instanceof Model) {
            Model mod = (Model) elem;

            EList<Profile> appliedProfiles = mod.getAppliedProfiles();

            if (appliedProfiles.size() == 1) {
                Profile prof = appliedProfiles.get(0);

                if (prof.eResource() != null) {
                    URI uri = prof.eResource().getURI();

                    if (!uri.isPlatformResource()) {
                        // Set a first class profile icon
                        return com.tibco.xpd.bom.resources.ui.Activator
                                .getDefault().getImageRegistry()
                                .get(BOMImages.BADGEMODEL_CM);
                    }
                }
            }
        }
        ...
        // Return the big icon
        return com.tibco.xpd.bom.resources.ui.Activator.getDefault()
                .getImageRegistry().get(BOMImages.BADGEMODEL);
        ...
        if (parser == null) {
            ...
            IAdaptable hintAdapter =
                    new UMLParserProvider.HintAdapter(
                            UMLElementTypes.Package_1007, getParserElement(),
                            parserHint);
