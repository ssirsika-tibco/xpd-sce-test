...
public class PrimTypeSuperTypeNameLabelEditPart extends CompartmentEditPart
     ...
     * @generated
     */
    public PrimTypeSuperTypeNameLabelEditPart(View view) {
        super(view);
    }
    ...
    protected void refreshLabel() {
        ...
        if (generalizationAdapter != null) {
            Classifier cls = (Classifier) ((View) getModel()).getElement();
            ...
            if (getParser() instanceof ISemanticParser) {
                ...
                if (modelParser.areSemanticElementsAffected(this
                    ...
                    if (event.getFeatureID(UMLPackage.class) == UMLPackage.GENERALIZATION_SET) {
                        ...
                        // Check that the supertype is in the same resource
                        // (diagram) as
                        // as the subtype. If it is then we don't want to draw a
                        // label
                        // because the generalization will be represented by a
                        // link line.
                        PrimitiveTypeEditPart ptep =
                                (PrimitiveTypeEditPart) getParent();
                        PrimitiveType subType =
                                (PrimitiveType) ptep.resolveSemanticElement();
                        List<Generalization> gens =
                                subType.getGeneralizations();

                        if (gens.size() > 0) {
                            Generalization gen =
                                    subType.getGeneralizations().get(0);
                            PrimitiveType superType =
                                    (PrimitiveType) gen.getGeneral();

                            if (superType != null && !superType.eIsProxy()) {
                                if (superType.eResource() == subType
                                        .eResource()) {
                                    ...
                                    if (BomUIUtil.isUserDiagram(diagramView)) {
                                        if (!BomUIUtil
                                            ...
                                            ptep
                                                    .getCustFigure()
                                                    .rebuildFigureWithSuperType(true);
                                            refreshLabel();
                                            return;
                                        }
    ...
    public void activate() {
        ...
        PrimitiveType primT = (PrimitiveType) ((View) getModel()).getElement();
        if (primT.getGeneralizations().size() > 0) {
            primT.getGeneralizations().get(0).eAdapters()
