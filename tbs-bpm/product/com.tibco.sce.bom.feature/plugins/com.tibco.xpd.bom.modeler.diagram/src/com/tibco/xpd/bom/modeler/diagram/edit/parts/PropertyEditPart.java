...
import com.tibco.xpd.bom.modeler.diagram.edit.policies.PropertyItemSemanticEditPolicy;
import com.tibco.xpd.bom.modeler.diagram.edit.policies.UMLTextNonResizableEditPolicy;
...
public class PropertyEditPart extends BOMStereoAwareCompartmentEditPart
    ...
    protected Image getLabelIcon() {
        ...
        if (parserElement != null) {
            return WorkingCopyUtil.getImage(parserElement);
        }
    ...
    protected String getLabelText() {
        ...
        // When the text label is retrieved, also set the tooltip
        if ((parserElement != null) && (parserElement instanceof Property)) {
            String tooltip = getTooltip((Property) parserElement);
            if (tooltip != null) {
                // Now set the tooltip on the property
                getFigure().setToolTip(new Label(tooltip));
            } else if (text != null) {
                // Set the tooltip to the text retrieved if previous value is
                // not found, better that it is set to something other than
                // "TEST"
    ...
    /**
     * Calculates the tooltip to display for a property
     * 
     * @param prop
     * @return
     */
    private String getTooltip(Property prop) {
        String text = null;

        Type propType = prop.getType();

        if ((propType != null) && (propType instanceof Classifier)) {
            Classifier cl = (Classifier) propType;
            if (cl != null && cl.eIsProxy() == false) {
                // Get the attribute name and add that first
                text = PrimitivesUtil.getDisplayLabel(prop) + " : "; //$NON-NLS-1$

                // If it is a primitive type, check if it is one of the base
                // primitive types, as we do not want the fully qualified
                // name for those
                if ((cl instanceof PrimitiveType)
                        && (PrimitivesUtil
                                .getBasePrimitiveType((PrimitiveType) cl) == cl)) {
                    text = text + PrimitivesUtil.getDisplayLabel(cl);
                } else {
                    text = text + cl.getQualifiedName();
                }
            } else {
    ...
    public ParserOptions getParserOptions() {
        // Show the attribute type when printing name
        return ParserOptions.SHOW_TYPE;
    }
        ...
        if (parser == null) {
            ...
            IAdaptable hintAdapter =
                    new UMLParserProvider.HintAdapter(
                            UMLElementTypes.Property_2001, getParserElement(),
                            parserHint);
    ...
    protected IFigure createFigure() {
        ...
        label.setToolTip(new Label("TEST"));
     ...
     * @generated
     */
    public class AttributeLabelFigure extends WrappingLabel {
        ...
        public AttributeLabelFigure() {
            this.setText(Messages.PropertyEditPart_attribute_figure_label);
        }
    ...
    /**
     * 
     * XPD-4660: Saket: Since eclipse has changed the method setSelected() in
     * AbstractEditPart to add an assertion, whenever we have the focus on a BOM
     * and then if we drag-drop an Attribute from the Palette to a class in that
     * model, we get an illegal argument exception stating: "An EditPart has to
     * be selectable (isSelectable() == true) in order to get selected". That's
     * why we are overriding isSelectable() in order to avoid that exception.
     * 
     */
    @Override
    ...
    public EditPart getTargetEditPart(Request request) {
        // We override this method to check to see if the user is doing a
        // marquee select and if they are we do not want to select attributes
        // within the class, this is because if we have attributes and classes
        // in the same selection it will not allow a copy. This seems to be the
        // built into the GMF layer as the default behaviour
        if (!(request instanceof SelectionRequest)
                && RequestConstants.REQ_SELECTION.equals(request.getType())) {
            return null;
        }
