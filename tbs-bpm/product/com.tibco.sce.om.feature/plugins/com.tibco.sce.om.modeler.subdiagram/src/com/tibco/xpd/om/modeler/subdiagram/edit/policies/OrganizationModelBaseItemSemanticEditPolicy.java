...
import com.tibco.xpd.om.modeler.subdiagram.edit.helpers.OrganizationModelBaseEditHelper;
import com.tibco.xpd.om.modeler.subdiagram.expressions.OrganizationModelAbstractExpression;
 ...
 * @generated
 */
public class OrganizationModelBaseItemSemanticEditPolicy extends
		SemanticEditPolicy {
	...
	public Command getCommand(Request request) {
		if (request instanceof ReconnectRequest) {
			...
			if (view instanceof View) {
				Integer id = new Integer(OrganizationModelVisualIDRegistry
		...
		if (elementType != null) {
			if (semanticCommand != null) {
				...
				completedRequest.setParameter(
						OrganizationModelBaseEditHelper.EDIT_POLICY_COMMAND,
						command);
			}
	...
	protected Command getDestroyElementCommand(View view) {
		...
		EditCommandRequestWrapper wrap = new EditCommandRequestWrapper(request,
		...
		return getDestroyElementCommandGen(view);
	}
	 ...
	 * @generated
	 */
	protected Command getDestroyElementCommandGen(View view) {
		EditPart editPart = (EditPart) getHost().getViewer()
				.getEditPartRegistry().get(view);
		DestroyElementRequest request = new DestroyElementRequest(
				getEditingDomain(), false);
	...
	public static class LinkConstraints {
		 ...
		 * @generated
		 */
		private static OrganizationModelAbstractExpression OrgUnitRelationship_3001_TargetExpression;

		/**
		 * @generated NOT
		 */
		public static boolean canCreateOrgUnitRelationship_3001(
				Organization container, OrgUnit source, OrgUnit target,
				boolean isHierarchical) {

			// Only one incoming hierarchical relationship per OrgUnit is
			// allowed
			if (target != null) {
				if (isHierarchical) {
					OrgUnitRelationship rel = target
					...
					// There might still be a circular relationship with an
					// OU further up the hierarchy tree. We need to traverse
					// up and see.
					if (OMUtil.isOrgUnitInHierarchy(source, target)) {
						return false;
					}
			...
			return canCreateOrgUnitRelationship_3001(container, source, target);
		}

		private static OrgUnit getParentOrgUnit(OrgUnit ou) {
		...
		public static boolean canCreateOrgUnitRelationship_3001(
				Organization container, OrgUnit source, OrgUnit target) {
			return canExistOrgUnitRelationship_3001(container, source, target);
		}

		/**
		 * @generated
		 */
		public static boolean canExistOrgUnitRelationship_3001(
				Organization container, OrgUnit source, OrgUnit target) {
			try {
				if (target == null) {
					return true;
				}
				if (OrgUnitRelationship_3001_TargetExpression == null) {
					Map env = Collections.singletonMap(OPPOSITE_END_VAR,
							OMPackage.eINSTANCE.getOrgUnit());
					OrgUnitRelationship_3001_TargetExpression = OrganizationModelOCLFactory
							.getExpression(
									"(self <> oppositeEnd)", OMPackage.eINSTANCE.getOrgUnit(), env); //$NON-NLS-1$
				}
				Object targetVal = OrgUnitRelationship_3001_TargetExpression
