...
import com.tibco.xpd.om.modeler.subdiagram.part.OrganizationModelLinkDescriptor;
 ...
 * @generated
 */
public class OrganizationSubCanonicalEditPolicy extends
	...
	protected List getSemanticChildrenList() {
		...
		for (Iterator it = OrganizationModelDiagramUpdater
				.getOrganization_79SemanticChildren(viewObject).iterator(); it
				.hasNext();) {
	...
	private Collection refreshConnections() {
		...
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
			...
			int diagramLinkVisualID = OrganizationModelVisualIDRegistry
			...
			for (Iterator LinkDescriptorsIterator = linkDescriptors.iterator(); LinkDescriptorsIterator
					.hasNext();) {
				OrganizationModelLinkDescriptor nextLinkDescriptor = (OrganizationModelLinkDescriptor) LinkDescriptorsIterator
						.next();
	...
	private Collection collectAllLinks(View view, Map domain2NotationMap) {
		...
		case OrganizationSubEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(OrganizationModelDiagramUpdater
						.getOrganization_79ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
		...
		case OrgUnitSubEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(OrganizationModelDiagramUpdater
						.getOrgUnit_1001ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case DynamicOrgUnitEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(OrganizationModelDiagramUpdater
						.getDynamicOrgUnit_1002ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
	...
	private Collection createConnections(Collection linkDescriptors,
		...
		for (Iterator linkDescriptorsIterator = linkDescriptors.iterator(); linkDescriptorsIterator
				.hasNext();) {
			final OrganizationModelLinkDescriptor nextLinkDescriptor = (OrganizationModelLinkDescriptor) linkDescriptorsIterator
					.next();
