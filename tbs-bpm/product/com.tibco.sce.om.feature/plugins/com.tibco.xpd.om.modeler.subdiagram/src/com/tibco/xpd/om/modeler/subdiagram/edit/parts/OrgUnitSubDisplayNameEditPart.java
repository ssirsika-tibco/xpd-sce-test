 ...
 * @generated
 */
public class OrgUnitSubDisplayNameEditPart extends CompartmentEditPart
	 ...
	 * @generated
	 */
	public OrgUnitSubDisplayNameEditPart(View view) {
		super(view);
	}
	...
	public void setLabel(WrappingLabel figure) {
		...
		if (semanticElement instanceof OrgUnit) {
			OrgUnit element = (OrgUnit) semanticElement;

			if (element != null) {
				// Check for any outgoing relationships. If we find any and one
				// is
				// hierarchical then we need to rebuild the figure to show the
				// +/-
				// icon
				EList<OrgUnitRelationship> outgoingRelationships = element
						.getOutgoingRelationships();
				OrgUnitSubEditPart parentEP = (OrgUnitSubEditPart) getParent();

				for (OrgUnitRelationship orgUnitRelationship : outgoingRelationships) {
					if (orgUnitRelationship.isIsHierarchical()) {

						Object model2 = parentEP.getModel();

						if (model2 instanceof Node) {
							Node node = (Node) model2;
							...
							for (Object object : sourceEdges) {
								if (object instanceof Edge) {
									...
									if (edge.getElement() == orgUnitRelationship) {
										if (edge.isVisible()) {
											parentEP
													.getPrimaryShape()
													.rebuildFigureWithExpandCollapseIcon(
															true, 0);
										} else {
											parentEP
													.getPrimaryShape()
				...
				if (element.getFeature() != null) {
					parentEP.getPrimaryShape().rebuildFigureWithFeatureLabel(
							true);
	...
	protected Image getLabelIcon() {
		...
		if (parserElement instanceof OrgUnit) {
			OrgUnit ou = (OrgUnit) parserElement;
	...
	protected void handleNotificationEvent(Notification event) {
		...
		if (event.getEventType() == Notification.SET) {
			if (event.getFeature() == OMPackage.Literals.ORG_UNIT__FEATURE) {
