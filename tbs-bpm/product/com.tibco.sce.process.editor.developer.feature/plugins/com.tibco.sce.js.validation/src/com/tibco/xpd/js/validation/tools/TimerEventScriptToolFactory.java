...
public class TimerEventScriptToolFactory extends JavaScriptToolFactory {

	public IPreProcessor createPreProcessor(IValidationScope scope, Object input) {
		IPreProcessor processor = null;
		if (input instanceof Activity) {
			Activity activity = (Activity) input;
			boolean isTimerEvent = ProcessScriptUtil.isTimerEventWithScriptType(activity,
					getScriptGrammar());
			if (isTimerEvent) {
				processor = new TimerEventScriptTool(activity);
			}
		}
		return processor;
	}
	...
	public Class<? extends IPreProcessor> getToolClass() {
		return TimerEventScriptTool.class;
	}
