...
/**
 * Script mapping compositor for REST service to process mappings. Contains
 ...
 * @since 10 Apr 2015
 */
public class JavaScriptRestToProcessMapping implements SingleMappingCompositor {
    ...
    public static class Factory extends ScriptMappingCompositorFactory {
        ...
        public ScriptMappingCompositor getCompositor(Activity activity,
                String target) {
            return new JavaScriptRestToProcessMapping(activity, target);
        }
        ...
        public ScriptMappingCompositor getCompositor(Activity activity,
            ...
            return new JavaScriptRestToProcessMapping(activity, target, script);
        }
     ...
     *            The target.
     */
    public JavaScriptRestToProcessMapping(Activity activity, String target) {
     ...
     * @return
     */
    private String getRestItemPath(Object source) {
        if (source instanceof RestParamTreeItem) {
            return ((RestParamTreeItem) source).getPath();
    ...
    public Collection<Object> getSourceItems(boolean isInput) {
        ...
        for (String name : getSourceItemNames()) {
            items.add(RestMapperTreeItemFactory.getInstance()
                    .resolveParameter(activity, direction, name));
        }
        return items;
    }
