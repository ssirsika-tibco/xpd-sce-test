 ...
 * 
 */
public class DeadlineExpiredScriptTaskScriptToolFactory extends JavaScriptToolFactory {

	public IPreProcessor createPreProcessor(IValidationScope scope, Object input) {
		IPreProcessor processor = null;
		if (input instanceof Activity) {
			Activity activity = (Activity) input;
			boolean b = ProcessScriptUtil.isActivityWithDeadlineExpiredScriptType(activity,
					getScriptGrammar());
			if (b) {
				processor = new DeadlineExpiredScriptTaskScriptTool(activity);
			}
		}
		return processor;
	}
	...
	public Class<? extends IPreProcessor> getToolClass() {
		return DeadlineExpiredScriptTaskScriptTool.class;
	}
