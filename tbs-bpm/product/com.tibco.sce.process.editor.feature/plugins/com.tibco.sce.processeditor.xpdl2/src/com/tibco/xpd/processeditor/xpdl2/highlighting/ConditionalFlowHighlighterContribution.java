...
/**
 * Highlighter for conditional sequence flow.
 * 
 * @author aallway
 * @since 8 Feb 2011
 */
public class ConditionalFlowHighlighterContribution extends
    ...
    public String getActivatedTooltipLabel() {
        return Messages.ConditionalFlowHighlighterContribution_ConditionalFlowHighlighter_tooltip;
    }
    ...
    public Collection<? extends EObject> getHighlightedDiagramModelObjects(
        ...
        List<Transition> conditionalFlow = new ArrayList<Transition>();

        for (Transition transition : Xpdl2ModelUtil
                .getAllTransitionsInProc(diagramProcess)) {
            Condition condition = transition.getCondition();
            if (condition != null) {
                ConditionType type = condition.getType();
                if (ConditionType.CONDITION_LITERAL.equals(type)
                        || ConditionType.OTHERWISE_LITERAL.equals(type)) {
                    conditionalFlow.add(transition);
                }
            }
        }
    ...
    public ImageDescriptor getMenuImageDescriptor() {
        return Xpdl2ProcessEditorPlugin
                .getImageDescriptor("icons/conditionalFlowMenu.png"); //$NON-NLS-1$
    }
    ...
    public String getMenuText() {
        return Messages.ConditionalFlowHighlighterContribution_ConditionalFlow_menu;
    }
