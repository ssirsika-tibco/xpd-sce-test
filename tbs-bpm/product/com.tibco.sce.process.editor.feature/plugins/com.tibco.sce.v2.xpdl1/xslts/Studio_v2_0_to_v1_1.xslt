...
<xsl:template match="xpdl2:DataField">
	<xpdl:DataField Id="{@Id}" Name="{@Name}">
		<xsl:copy-of select="@IsArray"/>
		
		<xsl:apply-templates select="*"/>
		
		<!-- Add extended attrs to carry length, precision and scale from v2 model. -->
		<xsl:variable name="declType" select="xpdl2:DataType/xpdl2:DeclaredType"/> <!-- may be decl type, may be basic type -->

		<!-- get basic type info from either the field itself or the type declaration of a typed field. -->
		<xsl:variable name="basicType" select="xpdl2:DataType/xpdl2:BasicType | /xpdl2:Package/xpdl2:TypeDeclarations/xpdl2:TypeDeclaration[@Id = $declType/@Id]/xpdl2:BasicType"/>
		
		<xsl:if test="$basicType">
			<xsl:variable name="length" select="$basicType/xpdl2:Length"/>
			<xsl:variable name="precision" select="$basicType/xpdl2:Precision"/>
			<xsl:variable name="scale" select="$basicType/xpdl2:Scale"/>
...
<!--
============================================
Ditch BasicType sub-elements (not suppoerted in v1)
============================================
-->
...
<!--
==============================================================================
Switch DeclaredType for data fields / params to equivalent basic types (declared types not handled in v1.
==============================================================================
-->
<xsl:template match="xpdl2:DeclaredType">
	<xsl:variable name="declId" select="@Id"/>
	<xsl:variable name="declType" select="/xpdl2:Package/xpdl2:TypeDeclarations/xpdl2:TypeDeclaration[@Id = $declId]"/>
	
	<xsl:choose>
		<xsl:when test="$declType/xpdl2:BasicType">
			<xpdl:BasicType>
				<xsl:apply-templates select="$declType/xpdl2:BasicType/@*"/>
			</xpdl:BasicType>
		</xsl:when>
		<xsl:otherwise>
...
<xsl:template match="xpdl2:WorkflowProcess">
	<xpdl:WorkflowProcess Id="{@Id}" Name="{@Name}">
		...
		<!-- We can make life A LOT easier for things handling v1 xpdl after us by 'pretending' that any interface parameters we iherit are part of out own. (and for ourselves here too. -->
						...
						<xsl:for-each select="$pools/xpdl2:Lanes/xpdl2:Lane">
							...
							<extensions:Lane Id="{$laneId}">
								...
								<xsl:for-each select="$notes">
									<xsl:variable name="gi" select="xpdl2:NodeGraphicsInfos/xpdl2:NodeGraphicsInfo[@ToolId = 'XPD']"/>
								...
								<extensions:IsDefault>true</extensions:IsDefault>
								<extensions:Size><xsl:value-of select="round(xpdl2:NodeGraphicsInfos/xpdl2:NodeGraphicsInfo[@ToolId = 'XPD']/@Height)"/></extensions:Size>
							</extensions:Lane>
						</xsl:for-each>
					...
					<xpdl:ExtendedAttribute Name="v2XPD_FormalParamExtra" Value="{$paramName}">
						<details>
							<xsl:attribute name="IsInterfaceParam">
								<xsl:choose>
									<xsl:when test="local-name(../..) = 'ProcessInterface'">true</xsl:when>
							...
							<xsl:attribute name="mandatory">
								<xsl:choose>
									<xsl:when test="@xpdExt:Mandatory = 'true'">true</xsl:when>
									<xsl:otherwise>false</xsl:otherwise>
								</xsl:choose> 
							</xsl:attribute>
	...
	<xsl:if test="$eventTaskBorderId = ''">
		...
		<xpdl:Activity Id="{@Id}" Name="{@Name}">
			...
			<xpdl:ExtendedAttributes>
				...
				<xpdl:ExtendedAttribute Name="XPD">
					<extensions:Activity LaneId="{xpdl2:NodeGraphicsInfos/xpdl2:NodeGraphicsInfo[@ToolId = 'XPD']/@LaneId}">
					...
					<xsl:when test="xpdl2:Event/xpdl2:IntermediateEvent">
						...
						<xsl:if test="count(xpdl2:Event/xpdl2:IntermediateEvent/xpdl2:TriggerResultLink) = 0">
							...
							<xsl:choose>
								...
								<xsl:when test="count(xpdl2:Event/xpdl2:IntermediateEvent/xpdl2:ResultError) > 0">
									<xsl:variable name="errorCode" select="xpdl2:Event/xpdl2:IntermediateEvent/xpdl2:ResultError/@ErrorCode"/>
									<xpdl:ExtendedAttribute Name="v2XPD_ThrowEventErrorCode" Value="{$errorCode}"/>
								</xsl:when>
							</xsl:choose>
						</xsl:if>
					</xsl:when>
					
					<!-- Preserve error code for End Events too -->
					<xsl:when test="xpdl2:Event/xpdl2:EndEvent/xpdl2:ResultError">
						<xsl:variable name="errorCode" select="xpdl2:Event/xpdl2:EndEvent/xpdl2:ResultError/@ErrorCode"/>
						<xpdl:ExtendedAttribute Name="v2XPD_ThrowEventErrorCode" Value="{$errorCode}"/>
					</xsl:when>
				...
				<!-- If this task has error event attached to border then preserve the error code(s). -->
				<xsl:variable name="errorEvents" select="$attachedEvents/xpdl2:ResultError"/>
				<xsl:for-each select="$errorEvents">
					<xsl:variable name="errorCode" select="@ErrorCode"/>
					<xpdl:ExtendedAttribute Name="v2XPD_CatchEventErrorCode" Value="{$errorCode}"/>
				</xsl:for-each>
				...
				<xsl:choose>
					<xsl:when test="@xpdExt:Visibility = 'Public'">
				...
				<!-- If this task has associated parameters then add them. -->
				<xsl:if test="xpdExt:AssociatedParameters/xpdExt:AssociatedParameter">
					<xpdl:ExtendedAttribute Name="Activity_xpdExt_AssociatedParameters">
						<xsl:for-each select="xpdExt:AssociatedParameters/xpdExt:AssociatedParameter">
							<AssociatedParameter>
				...
				<xsl:if test="xpdExt:DurationCalculation">
					<xpdl:ExtendedAttribute Name="Activity_xpdExt_DurationCalculation">
						<xsl:call-template name="CopyNodeWithoutPrefix">
							<xsl:with-param name="input" select="xpdExt:DurationCalculation"/>
						</xsl:call-template>
					</xpdl:ExtendedAttribute>
				</xsl:if>

				<!-- Copy iProcessExt:ConditionProperties out to ext attrs. -->
				<xsl:if test="iProcessExt:ConditionProperties">
					<xpdl:ExtendedAttribute Name="Activity_iProcessExt_ConditionProperties">
						<xsl:call-template name="CopyNodeWithoutPrefix">
							<xsl:with-param name="input" select="iProcessExt:ConditionProperties"/>
						</xsl:call-template>
					</xpdl:ExtendedAttribute>
				</xsl:if>
		...
		<xsl:for-each select="$activities">
			...
			<xsl:choose>
				...
				<xsl:when test="$eventTaskBorderId != ''">
					<xsl:if test="xpdl2:Event/xpdl2:IntermediateEvent/xpdl2:ResultCompensation">
				...
				<xsl:when test="xpdl2:Event/xpdl2:IntermediateEvent/xpdl2:TriggerResultLink/@LinkId != ''">
					<xsl:variable name="tgtId" select="xpdl2:Event/xpdl2:IntermediateEvent/xpdl2:TriggerResultLink/@LinkId"/>
					<xsl:variable name="target" select="$activities[@Id = $tgtId]"/>
					
					<xsl:if test="$target">
						<xpdl:Transition Id="{@Id}_to_{$tgtId}" From="{@Id}" To="{xpdl2:Event/xpdl2:IntermediateEvent/xpdl2:TriggerResultLink/@LinkId}" Name="Link From '{@Name}' to '{$target/@Name}'"/>
					</xsl:if>
				</xsl:when>
...
<xsl:template match="xpdl2:Transition">
	...
	<xpdl:Transition Id="{@Id}" From="{$realFromId}" To="{@To}">
					...
					<xpdl:Condition>
						...
						<xsl:choose>
							<xsl:when test="xpdl2:Condition/xpdl2:Expression">
								<!-- TODO - Convert from studio java script to staffware expression -->
								<xpdl:Xpression><xsl:value-of select="xpdl2:Condition/xpdl2:Expression"/></xpdl:Xpression>
							</xsl:when>
		...
		<xsl:if test="$eventTaskBorderId != '' or count(xpdl2:ExtendedAttributes/xpdl2:ExtendedAttribute) > 0 or $srcAct/xpdl2:Event/xpdl2:IntermediateEvent/xpdl2:TriggerTimer">
			<xpdl:ExtendedAttributes>
				...
				<xsl:choose>
					<xsl:when test="$eventTaskBorderId != ''">
						...
						<!-- If the source object is error event on task border then flag it -->
						<xsl:if test="$srcAct/xpdl2:Event/xpdl2:IntermediateEvent/xpdl2:ResultError">
							<xpdl:ExtendedAttribute Name="v2XPD_FlowFromCatchError" Value="true"/>
						</xsl:if>
...
<xsl:template name="getV1ActivityType">
	...
	<xsl:choose>
		...
		<xsl:when test="$activity/xpdl2:Route/@GatewayType = 'XOR'">
			<xsl:choose>
				<xsl:when test="$activity/xpdl2:Route/@Instantiate = 'true'">
		...
		<xsl:when test="$activity/xpdl2:Route/@GatewayType = 'OR'">
			<xsl:text>GATEWAY_OR</xsl:text>
		</xsl:when>

		<xsl:when test="$activity/xpdl2:Route/@GatewayType = 'AND'">
...
<xsl:template name="CopyNodeWithoutPrefix">
	...
	<xsl:variable name="elName" select="local-name($input)"/>
