...
/**
 * Resolution to set the error code (and hence fault name) of a throw fault
 * message error event.
 * 
 * @author aallway
 * @since 3.3
 */
public class SetThrowFaultNameResolution extends AbstractWorkingCopyResolution {
        ...
        if (target instanceof Activity) {
            ...
            EnterValueDialog nameDialog =
                    new EnterValueDialog(
                            ...
                            Messages.SetThrowFaultNameResolution_SetFaultName_label,
                            Messages.SetThrowFaultNameResolution_FaultName_label,
                            "") { //$NON-NLS-1$
                        @Override
                        protected String validateValue(String newValue) {
                            if (newValue == null || newValue == "") { //$NON-NLS-1$
                                return Messages.SetThrowFaultNameResolution_MustHaveFaultName_longdesc;
                            } else {
