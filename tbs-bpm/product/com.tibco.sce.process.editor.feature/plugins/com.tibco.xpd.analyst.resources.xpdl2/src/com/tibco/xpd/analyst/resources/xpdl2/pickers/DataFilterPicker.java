...
import com.tibco.xpd.analyst.resources.xpdl2.providers.IDataPickerProvider;
import com.tibco.xpd.analyst.resources.xpdl2.providers.IDataPickerProvider.DataType;
import com.tibco.xpd.analyst.resources.xpdl2.utils.ProcessUIUtil;
import com.tibco.xpd.resources.util.WorkingCopyUtil;
import com.tibco.xpd.xpdl2.NamedElement;

/**
 * Data type picker. This will use the indexer to acquire (proxy) information
 * about relevant <code>EObject</code>s and will resolve to the actual
 * <code>EObject</code> when a selection is made (by pressing OK in the dialog),
 * the proxy item contains the actual EObject.
 * <p>
 * Use {@link #addFilter(IFilter) addFilter} to filter the content of the
 * picker. Alternatively, the constructor {@link #DataPicker(Shell, EObject[])}
 * can be used to provide a list of <code>EObject</code>s to exclude from the
 * picker.
 * </p>
 * <p>
 ...
 * 
 */
public class DataFilterPicker extends BaseFilterPicker {
    ...
    public enum DataPickerType {
         ...
         * Data field picker
         */
        DATAFIELD(Messages.DataFilterPicker_DataPicker_SelectDataField_Title,
                "DataPicker.dataField"), //$NON-NLS-1$
        /**
         * Formal Parameter type picker
         */
        FORMALPARAMETER(
                Messages.DataFilterPicker_DataPicker_SelectFormalParameter_Title,
                "DataPicker.formalParameter"), //$NON-NLS-1$
        /**
         * Data field and Formal Parameter picker
         */
        DATAFIELD_FORMALPARAMETER(
                Messages.DataFilterPicker_DataPicker_SelectDataFieldOrFormalParameter_Title,
                "DataPicker.dataFieldFormalParameter"), //$NON-NLS-1$ 
        /**
         * Participants picker
         */
        PARTICIPANTS(
                Messages.DataFilterPicker_DataPicker_SelectParticipants_Title,
                "DataPicker.participants"); //$NON-NLS-1$ 
        ...
        private final String message =
                Messages.DataFilterPicker_Dialog_Display_Message;

        DataPickerType(String title, String dialogSettingId) {
            this.title = title;
    ...
    private final DataPickerType pickerType;

    private Comparator<IDataPickerProxyItem> comparator;

    private final IDataPickerProvider provider;

    private final DataPickerLabelProvider labelProvider;

    private Object[] content = null;

    public DataFilterPicker(Shell shell, DataPickerType type, boolean multi) {
        super(shell, multi);
        Assert.isNotNull(type, "Data Picker type"); //$NON-NLS-1$
        this.pickerType = type;
        filters = new HashSet<IFilter>();
        provider = DataFilterPickerProviderHelper.getInstance();
        Assert.isNotNull(provider, "Data Picker provider"); //$NON-NLS-1$
        ...
        labelProvider = new DataPickerLabelProvider();
        setListLabelProvider(labelProvider);
        setListSelectionLabelDecorator(labelProvider);
        setDetailsLabelProvider(new DataPickerDetailsLabelProvider());

    }

    /**
     * Constructor. Data Picker base class.
     * 
     * @param shell
     *            parent shell
     * @param exclude
     ...
     */
    public DataFilterPicker(Shell shell, EObject[] exclude,
        ...
        if (exclude != null && exclude.length > 0) {
            ...
            addFilter(new IFilter() {
                public boolean select(Object toTest) {
                    ...
                    if (toTest instanceof IDataPickerProxyItem) {
                        URI uri = ((IDataPickerProxyItem) toTest).getURI();
    ...
    public void setInitialSelections(Object[] selectedElements) {
        Set<IDataPickerProxyItem> pickerProxyItems =
                new HashSet<IDataPickerProxyItem>();
        if (selectedElements != null && getScope() != null) {
            ...
            for (Object obj : selectedElements) {
                if (obj instanceof EObject) {
                    ...
                    pickerProxyItems.add((IDataPickerProxyItem) obj);
                }
            }
            pickerProxyItems.addAll(DataFilterPickerProviderHelper
                    .getDataIndexerItemList(elementList, getScope()));
        }
     ...
     * passed to the filter will be of type <code>IDataPickerProxyItem</code>.
     * 
     * @param filter
     */
    ...
    protected ItemsFilter createFilter() {
        return new ShowAllWhenEmptyItemsFilter() {
            ...
            public boolean isConsistentItem(Object item) {
                ...
                if (item instanceof IDataPickerProxyItem) {
    ...
    protected Object[] getContent(EObject scope) {
        if (content == null) {
            Set<IDataPickerProxyItem> itemsList =
                    new HashSet<IDataPickerProxyItem>();
            if (provider != null) {

                // Get all data fields
                if (pickerType == DataPickerType.DATAFIELD) {
                    itemsList.addAll(Arrays.asList(provider
                            .getContent(DataType.DATAFIELD, scope)));
                }

                // Get all formal parameters
                if (pickerType == DataPickerType.FORMALPARAMETER) {
                    itemsList.addAll(Arrays.asList(provider
                            .getContent(DataType.FORMALPARAMETER, scope)));
                }

                // Get all data fields and formal parameters
                if (pickerType == DataPickerType.DATAFIELD_FORMALPARAMETER) {
                    itemsList.addAll(Arrays.asList(provider
                            .getContent(DataType.DATAFIELD_FORMALPARAMETER,
                                    scope)));
                }

                // Get all participants
                if (pickerType == DataPickerType.PARTICIPANTS) {
                    itemsList.addAll(Arrays.asList(provider
                            .getContent(DataType.PARTICIPANTS, scope)));
                }
            ...
            content =
                    itemsList
                            .toArray(new IDataPickerProxyItem[itemsList.size()]);
            content = content != null ? content : new IDataPickerProxyItem[0];
        }
        return content;
    }
    ...
    public Object[] getResult() {
        ...
        if (result != null) {
            for (Object obj : result) {
                if (obj instanceof IDataPickerProxyItem) {
                    IDataPickerProxyItem item = (IDataPickerProxyItem) obj;
    ...
    protected Comparator getItemsComparator() {
        if (comparator == null) {
            comparator = new Comparator<IDataPickerProxyItem>() {

                public int compare(IDataPickerProxyItem o1,
                        IDataPickerProxyItem o2) {
                    int ret = 0;
                    if (o1 == o2) {
                        ret = 0;
                    } else {
     ...
     *         found.
     */
    protected String getQualification(IDataPickerProxyItem item) {
        ...
        /*return new SelectionHistory() {
            ...
            protected Object restoreItemFromMemento(IMemento memento) {
                ...
                if (uriStr != null && name != null) {
                    URI elementURI =
                            URI.createPlatformResourceURI(uriStr, false);
                    if (provider != null) {
                        if (isValidHistoryItem(uriStr)) {
                            item =
                                    provider.getItem(elementURI,
                                            name,
                                            getScope());
                        }
                    }
                } else if (name != null) {
                    item = getExtraItem(name);
                }
                return item;
            }
            ...
            protected void storeItemToMemento(Object item, IMemento memento) {
                if (item instanceof IDataPickerProxyItem) {
                    IDataPickerProxyItem proxyItem =
    ...
    private class DataPickerLabelProvider extends LabelProvider implements
            ILabelDecorator {
        ...
        public String getText(Object element) {
            ...
            if (element instanceof IDataPickerProxyItem) {
                if (((IDataPickerProxyItem) element).getItem() instanceof NamedElement) {
                    text =
                            WorkingCopyUtil
                                    .getText((NamedElement) ((IDataPickerProxyItem) element)
                                            .getItem());
                } else {
                    text = ((IDataPickerProxyItem) element).getName();
                }
        ...
        public Image getImage(Object element) {
            ...
            if (element instanceof IDataPickerProxyItem) {
                img = ((IDataPickerProxyItem) element).getImage();
            }
        ...
        public String decorateText(String text, Object element) {
            if (element instanceof IDataPickerProxyItem) {
                IDataPickerProxyItem item = (IDataPickerProxyItem) element;

                text = String.format("%s - %s", text, //$NON-NLS-1$
    ...
    private class DataPickerDetailsLabelProvider extends LabelProvider {
        ...
        public String getText(Object element) {
            ...
            if (element instanceof IDataPickerProxyItem) {
                URI uri = ((IDataPickerProxyItem) element).getURI();
        ...
        public Image getImage(Object element) {
            ...
            if (element instanceof IDataPickerProxyItem) {
                Object item = ((IDataPickerProxyItem) element).getItem();
