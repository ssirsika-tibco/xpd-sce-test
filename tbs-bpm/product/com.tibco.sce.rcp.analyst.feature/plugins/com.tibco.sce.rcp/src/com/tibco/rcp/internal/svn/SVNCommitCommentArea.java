...
/**
 * copy of org.tigris.subversion.subclipse.ui.comments.CommitCommentArea with
 * some minor modifications
 * 
 * @author kupadhya
 * @since 22 Jan 2013
 */
public class SVNCommitCommentArea extends DialogArea {
    private boolean showLabel = true;;

    public static final String SPELLING_ERROR = "spelling.error"; //$NON-NLS-1$

    private class TextBox implements ModifyListener, TraverseListener,
        ...
        public TextBox(Composite composite, String message, String initialText) {
            ...
            fMessage = message;
            fText = initialText;
            // Create a resource manager for the composite so it gets
            ...
            AnnotationRulerColumn annotationRuler =
                    new AnnotationRulerColumn(annotationModel, 16,
                            annotationAccess);
            ...
            SourceViewer sourceViewer =
                    new SourceViewer(cc, compositeRuler, null, true, SWT.MULTI
            ...
            if (isSpellingAnnotationEnabled()) {
                ...
                AnnotationPainter ap =
                        new AnnotationPainter(sourceViewer, annotationAccess);
                ap.addAnnotationType(SPELLING_ERROR);
                ap.setAnnotationTypeColor(SPELLING_ERROR,
                        getSpellingErrorColor(composite));
            ...
            // front)
            sourceViewer.configure(new SourceViewerConfig(annotationModel,
                    document));
        ...
        private boolean mustWrapWord() {
            if (commentProperties != null
                    && commentProperties.getLogWidthMarker() > 0) {
                return false;
            }
            return true;
        }
        ...
        private boolean isSpellingAnnotationEnabled() {
            // Need to determine how to ask the proper question to the
        ...
        private Color getSpellingErrorColor(Composite composite) {
            AnnotationPreference pref =
                    EditorsUI
                            ...
                            .getAnnotationPreference("org.eclipse.ui.workbench.texteditor.spelling"); // $NON-NLS-1$
            String preferenceKey = pref.getColorPreferenceKey();
            try {
                return fResources
                        .createColor(PreferenceConverter.getColor(EditorsUI
                                .getPreferenceStore(), preferenceKey));
            } catch (DeviceResourceException e) {
                SVNUIPlugin.log(IStatus.ERROR,
                        Messages.SVNCommitCommentArea_internal,
                        e);
        ...
        public int getCommentLength() {
            if (fTextField == null)
                return 0;
            if (fTextField.getText()
                    .equals(Messages.SVNCommitCommentArea_CommitCommentArea_0))
                return 0;
    ...
    public class SourceViewerConfig extends SourceViewerConfiguration {
         ...
         * 
         * @see
         * org.eclipse.jface.text.source.SourceViewerConfiguration#getTextHover
    ...
    public class CommentSpellingReconcileStrategy implements
        ...
        public CommentSpellingReconcileStrategy(AnnotationModel annotationModel) {
            ...
            fSpellingContext.setContentType(Platform.getContentTypeManager()
        ...
        public void reconcile(IRegion region) {
            SpellingProblemCollector collector =
                    new SpellingProblemCollector(fAnnotationModel);
            EditorsUI.getSpellingService().check(fDocument,
                    fSpellingContext,
                    collector,
                    null);
        }
        ...
        private class SpellingProblemCollector implements
             ...
             * accept(org.eclipse.ui.texteditor.spelling.SpellingProblem)
             */
            @Override
            public void accept(SpellingProblem problem) {
                fAddAnnotations.put(new Annotation(SPELLING_ERROR, false,
                        problem.getMessage()), new Position(
            ...
            public void endCollecting() {
                ...
                for (Iterator iter = fAnnotationModel.getAnnotationIterator(); iter
                        .hasNext();) {
                ...
                for (Iterator iter = removeAnnotations.iterator(); iter
                        .hasNext();)
                    fAnnotationModel.removeAnnotation((Annotation) iter.next());
                for (Iterator iter = fAddAnnotations.keySet().iterator(); iter
                        .hasNext();) {
                    Annotation annotation = (Annotation) iter.next();
    ...
    private static class ComboBox extends Observable implements
        ...
        private void populateList() {
            ...
            for (int i = 0; i < fCommentTemplates.length; i++) {
                fCombo.add(Messages.SVNCommitCommentArea_CommitCommentArea_6
                        + ": " + //$NON-NLS-1$
    ...
    private static final String EMPTY_MESSAGE =
            Messages.SVNCommitCommentArea_CommitCommentArea_0;

    private static final String COMBO_MESSAGE =
            Messages.SVNCommitCommentArea_CommitCommentArea_1;
     ...
     * @param settings
     */
    public SVNCommitCommentArea(Dialog parentDialog, IDialogSettings settings) {
     ...
     */
    public SVNCommitCommentArea(Dialog parentDialog, IDialogSettings settings,
            String enterCommentMessage) {
        this(parentDialog, settings);
        this.enterCommentMessage = enterCommentMessage;
    }

    public SVNCommitCommentArea(Dialog parentDialog, IDialogSettings settings,
            String enterCommentMessage, CommentProperties commentProperties) {
    ...
    public Control createArea(Composite parent) {
        ...
        if (showLabel) {
            ...
            if (enterCommentMessage == null)
                label.setText(Messages.SVNCommitCommentArea_ReleaseCommentDialog_enterComment);
            else
                label.setText(enterCommentMessage);
        }
        ...
        final String[] comments =
                SVNUIPlugin.getPlugin().getRepositoryManager()
                        .getCommentsManager().getPreviousComments();
        final String[] commentTemplates =
                SVNUIPlugin.getPlugin().getRepositoryManager()
                        .getCommentsManager().getCommentTemplates();
        fComboBox =
                new ComboBox(fComposite, COMBO_MESSAGE, comments,
                        commentTemplates);
    ...
    void openCommentTemplatesPreferencePage() {
        PreferencesUtil
                .createPreferenceDialogOn(null,
                        ...
                        null).open();
        fComboBox.setCommentTemplates(SVNUIPlugin.getPlugin()
                .getRepositoryManager().getCommentsManager()
                .getCommentTemplates());
    }
    ...
    public String getCommentWithPrompt(Shell shell) {
        ...
        if (comment.length() == 0) {
            ...
            final String value =
                    store.getString(ISVNUIConstants.PREF_ALLOW_EMPTY_COMMIT_COMMENTS);
            ...
            if (MessageDialogWithToggle.PROMPT.equals(value)) {
                ...
                final String title =
                        Messages.SVNCommitCommentArea_CommitCommentArea_2;
                final String message =
                        Messages.SVNCommitCommentArea_CommitCommentArea_3;
                final String toggleMessage =
                        Messages.SVNCommitCommentArea_CommitCommentArea_4;
                ...
                final MessageDialogWithToggle dialog =
                        MessageDialogWithToggle
                                .openYesNoQuestion(shell,
                                        ...
                                        false,
                                        store,
                                        ISVNUIConstants.PREF_ALLOW_EMPTY_COMMIT_COMMENTS);
    ...
    @Override
    protected void firePropertyChangeChange(String property, Object oldValue,
            Object newValue) {
    ...
    private String getCommitTemplate() throws SVNException {
        if ((commentProperties != null)
                && (commentProperties.getLogTemplate() != null)) {
