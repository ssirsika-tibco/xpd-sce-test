...
public class CustomCompletionProposal implements ICompletionProposal, ICompletionProposalExtension, ICompletionProposalExtension2, IRelevanceCompletionProposal {
	...
	protected int fRelevance = CustomCompletionProposal.R_NONE;
		...
		if (caretOffset == getReplacementOffset()) {
			...
			try {
				...
				if (caretOffset > getReplacementOffset()) {
					...
					int endPosition = preCaretTextLength;
					String replaceString = getReplacementString();
					if(endPosition > replaceString.length()){
					    endPosition = replaceString.length();
					}
					document.replace(getReplacementOffset(), preCaretTextLength, replaceString.substring(0, endPosition));
				}
