...
public class JScriptDotExpressionValidator extends AbstractExpressionValidator {
            ...
            if (expr instanceof AST && token instanceof Token) {
                ...
                case JScriptTokenTypes.DOT:
                    IValidateResult processDotExpression =
                            processDotExpression(expression,
                                    astExpression,
                                    (Token) token);
    ...
    private IValidateResult processDotExpression(IExpr expression,
            AST astExpression, Token token) {
            ...
            if (firstChild.getFirstChild() == null) {
                ...
                if (delegateEvaluateExpression != null) {
                    ...
                    if (firstChild.getType() == JScriptTokenTypes.IDENT) {
                        ...
                        /*
                         * Check if it's a statically contributed JSCLass from
                         * UML.
                         */
                        if (JScriptUtils.isASupportedClass(identExpression,
                                getSupportedJsClasses(getInfoObject()))) {
                            isStaticReference = true;

                        } else {
                            /*
                             * Else it might be a scriptRelevant data that
    ...
    private IValidateResult processExpressionAfterDot(IExpr expression,
            IScriptRelevantData type, AST astExpression, AST firstChild,
            Token token, boolean isStaticReference) {
        if (type != null) {
                ...
                if (nextSibling.getType() == JScriptTokenTypes.IDENT) {
                    if (areValidResolutionTypes(resolvedTypes)) {
                        ...
                        if (astExpression.getNextSibling() != null
                            ...
                            parameters =
                                    resolveMethodParameters(astExpression.getNextSibling(),
                                            token);
                        }
                        ...
                        if (type.getType() != null
                                && type.getType()
                                        .equals(JsConsts.VOID_DATA_TYPE)) {
                            String message =
                                    Messages.ExpressionValidator_Operation_Void;
                    ...
                    && type.getType().equals(JsConsts.VOID_DATA_TYPE)) {
    ...
    private IValidateResult processOwnedFeature(IExpr expression,
            String dataType, String propertyOrMethodName,
            List<IScriptRelevantData> umlTypes,
    ...
    private IValidateResult processProperty(IExpr expression, String dataType,
            String propertyName, List<IScriptRelevantData> umlTypes,
            Map<String, IScriptRelevantData> parameters, Token token,
            boolean isStaticReference) {
        ...
        for (IScriptRelevantData umlType : umlTypes) {
            if (umlType instanceof IUMLScriptRelevantData) {
                matchingPropertyType =
                        getMatchingPropertyType((IUMLScriptRelevantData) umlType,
                                propertyName,
                                isStaticReference);
        ...
        if (matchingPropertyType != null) {
            ...
            return updateResult(expression,
                    ...
                    createGenericContext(matchingPropertyType,
                            isGenericContextArray(matchingPropertyType,
                                    matchingPropertyType)));
        } else {
            // No property found
            String message =
                    Messages.ExpressionValidator_PropertyInvalid_For_Data_Type;
    ...
    private IValidateResult processMethod(IExpr expression, String dataType,
            String methodName, List<IScriptRelevantData> umlTypes,
            Map<String, IScriptRelevantData> parameters, Token token,
            boolean isStaticReference) {
        ...
        IScriptRelevantData currentGenericContext =
                JScriptUtils.getCurrentGenericContext(umlTypeContext);

        List<JsMethod> methodsWithName =
                getMethodsWithName(umlTypes, methodName);
        if (methodsWithName == null || methodsWithName.isEmpty()) {
            // No method found
            String message =
                    Messages.ExpressionValidator_MethodInvalid_For_Data_Type;
            List<String> additionalAttributes = new ArrayList<String>();
            ...
            List<JsMethod> methodsWithParamNumber =
                    getMethodsWithParamNumber(methodsWithName, parametersNumber);
            if (methodsWithParamNumber == null
                    || methodsWithParamNumber.isEmpty()) {
                // param number don't match
                String message =
                        Messages.ExpressionValidator_MethodParamsNumberInvalid_For_Data_Type;
                List<String> additionalAttributes = new ArrayList<String>();
                additionalAttributes.add(methodName);
                addErrorMessage(token, message, additionalAttributes);
                return null;
                ...
                if (methodsWithMatchingParamTypes == null
                    ...
                    String message =
                            Messages.ExpressionValidator_MethodParamsTypeInvalid_For_Data_Type;
                    List<String> additionalAttributes = new ArrayList<String>();
                    additionalAttributes.add(methodName);
                    ...
                    JsMethod matchMethod =
                            methodsWithMatchingParamTypes.iterator().next();
                    if (matchMethod != null) {
                        ...
                        if (returnType != null) {
                            ...
                            if (getUmlClass(returnType) != null) {
                                ...
                                /*
                                 * XPD-3129 ScriptrelevantData is used to
                                 * resolve local var of case ref types which are
                                 * otherwise resolved using UMLClass and for
                                 * Case Ref types they do not have any UMLClass
                                 * representation
                                 */
                                createScriptRelevantData =
                                        returnType.getScriptRelevantData();
                            } else {
                                createScriptRelevantData =
                                        createScriptRelevantData(returnType.getType(),
                                                paramDataType,
                                                returnType.canRepeat(),
                                                currentGenericContext,
                                                returnType);
                            ...
                            createScriptRelevantData =
                                    createScriptRelevantData(JsConsts.VOID_DATA_TYPE,
                        ...
                        if (createScriptRelevantData != null) {
                            if (!isSupportedMethod(umlTypes,
                                    methodName,
                                    token,
                                    parameters)) {
                                String message =
                                        Messages.ExpressionValidator_MethodNotSupported;
                            ...
                            if (createScriptRelevantData != null
                                    && createScriptRelevantData.getType() != null
                                    && (JScriptUtils
                                            .isGenericType(createScriptRelevantData
                                                    .getType()) || JScriptUtils
                                            .isContextlessType(createScriptRelevantData))) {
                                newGenericContext = currentGenericContext;
                            } else {
                                newGenericContext =
                                        createGenericContext(createScriptRelevantData,
                                                isGenericContextArray(createScriptRelevantData,
                                                        createScriptRelevantData));
                            }
                            ...
                            if (getCurrentSpecialGenericContextForMethod(newGenericContext,
                                    ...
                                    parameters) != null) {
                                newGenericContext =
                                        getCurrentSpecialGenericContextForMethod(newGenericContext,
                                                matchMethod,
                                                parameters);
    ...
    protected void performSpecificMethodValidation(String propertyName,
            IScriptRelevantData currentGenericContext,
            IScriptRelevantData newGenericContext, JsMethod matchMethod,
            IScriptRelevantData returnType, boolean isArrayContext,
            Map<String, IScriptRelevantData> parameters, Token token) {
        if (propertyName != null
                && propertyName.equals("addAll") && JScriptUtils.isXsdAnyType(currentGenericContext) && parameters != null && parameters.size() == 1) {//$NON-NLS-1$
            IScriptRelevantData parameter =
                    parameters.values().iterator().next();
            if (parameter != null
                    && !JScriptUtils.isDynamicComplexType(parameter)) {
                String message =
                        Messages.ExpressionValidator_MethodParamsTypeInvalid_For_Data_Type;
                List<String> additionalAttributes = new ArrayList<String>();
                additionalAttributes.add(propertyName);
    ...
    /**
     * Validates a UML Property that is an array and ensures no null arguments
     * can be added or set
     * 
     * @param propertyName
     * @param parameters
     ...
     * @return
     */
    private boolean validateUMLArrayNullParams(String propertyName,
            Map<String, IScriptRelevantData> parameters, Token token,
            ...
            for (IScriptRelevantData data : parameters.values()) {
                if (data.getName().equals("null")) { //$NON-NLS-1$
                    if (currentGenericContext instanceof ITypeResolution) {
                        Object extendedInfo =
                                ((ITypeResolution) currentGenericContext)
                                        .getExtendedInfo();
                        if (extendedInfo instanceof DefaultJsReference) {
                            Element element =
                                    ((DefaultJsReference) extendedInfo)
                                            .getElement();
    ...
    protected void validateStatic(boolean isStaticReference,
            boolean isStaticMethod, String propertyName, Token token) {
        if (isStaticReference && !isStaticMethod) {
            String message =
                    Messages.ExpressionValidator_StaticCallToNonStaticMethod;
            List<String> additionalAttributes = new ArrayList<String>();
            additionalAttributes.add(propertyName);
            addErrorMessage(token, message, additionalAttributes);
        } else if (!isStaticReference && isStaticMethod) {
            String message =
                    Messages.ExpressionValidator_NonStaticCallToStaticMethod;
            List<String> additionalAttributes = new ArrayList<String>();
            additionalAttributes.add(propertyName);
    ...
    private boolean areValidResolutionTypes(
    ...
    protected void addWarningMessage(Token token, String warningMessage,
            List<String> additionalAttributes) {
        /*
         * XPD-4375: overriding this method for Forms to have scope on this
         * method
         */
