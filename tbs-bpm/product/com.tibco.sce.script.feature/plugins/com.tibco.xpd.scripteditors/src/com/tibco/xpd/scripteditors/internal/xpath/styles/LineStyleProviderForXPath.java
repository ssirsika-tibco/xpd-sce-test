...
import com.tibco.xpd.script.sourceviewer.internal.contentassist.LexerCacheForJavaScript;
...
public class LineStyleProviderForXPath implements LineStyleProvider,
        IDocumentListener {
    ...
    public LineStyleProviderForXPath() {
        super();
    }
    ...
    public void init(IStructuredDocument structuredDocument,
        ...
        XPathColorPreferences.addPropertyChangeListener(fPreferenceListener);
    }
    ...
    protected ILexer getParseStartPoint(IStructuredDocumentRegion fnode,
            ITextRegion region, int offStart, boolean startHere) {
        int iNodeOffset = 0;
        ...
        if (nd.pcParseCache == null) {
            ...
            if (fnode != null) {
                ...
                // strOldNodeValue);
                nd.pcParseCache =
                        LexerCacheForJavaScript.getCache(fnode,
                                nd.strOldNodeValue);
                ...
                nd.pcParseCache =
                        LexerCacheForJavaScript
                                .getCache(fnode,
                                        nd.strOldNodeValue == null ? "" : nd.strOldNodeValue); //$NON-NLS-1$
            }
        }
    ...
    public void documentChanged(DocumentEvent event) {
        ...
        while (keys.hasMoreElements()) {
            IStructuredDocumentRegion fnode =
                    (IStructuredDocumentRegion) keys.nextElement();
    ...
    public void nodeChanged(IStructuredDocumentRegion fnode, int idxInsertPos,
            int iReplacedLen, String strInserted) {
        nodedata nd = getNodeData(fnode);
        try {
            if (fnode != null) {
                LexerCacheForJavaScript lx =
                        LexerCacheForJavaScript.getCache(fnode, ""); //$NON-NLS-1$
                if (nd.pcParseCache == null) {
                    // do nothing
                } else {
                    if (nd.pcParseCache != lx) {
                        Logger
                                .log(Logger.WARNING_DEBUG,
                                        "Unreliable way to find the node, and memory leak (hopefully small) will result, unless only showing part of editor (then everything is fine)"); //$NON-NLS-1$
            ...
            if (strInserted != null) {
                String nv = null;
                nv =
                        nd.strOldNodeValue.substring(0, idxInsertPos)
                                + strInserted
                                + nd.strOldNodeValue.substring(idxInsertPos
                                        + iReplacedLen);
                if (nv != null) {
                    if (nd.strOldNodeValue == null) {
                        ...
                        if (false) {
                            int imax =
                                    (nv.length() < nd.strOldNodeValue.length()) ? nv
                                            .length()
                                            : nd.strOldNodeValue.length();
                            while (idx < imax) {
                                if (nv.charAt(idx) != nd.strOldNodeValue
                                        .charAt(idx))
                                    break;
                                idx++;
                            }
                        ...
                        nd.pcParseCache.notifyChange(nv,
                                idx,
                                nd.strOldNodeValue.length() - idx,
                                nv.length() - idx);
                        if (nd.cachedStyles.size() > 0) {
                            int csize = nd.cachedStyles.size();
                            int offIdx = idx + nd.offCachedStylesRegion;
                            StyleRange srOne =
                                    (StyleRange) (nd.cachedStyles.get(0));
                            if (srOne.start > offIdx) {
                                nd.cachedStyles.clear();
                            } else {
                                while ((csize--) > 0) {
                                    srOne =
                                            (StyleRange) nd.cachedStyles
                                                    .get(csize);
            ...
            Logger
                    .log(Logger.WARNING_DEBUG,
                            "Exception in notifyChanged() of LineStyleProviderForJavaScript", exc); //$NON-NLS-1$
        }
    }

    /**
	 */
    public boolean prepareRegions(ITypedRegion currentRegion, int offStart,
            int length, Collection holdResults) {
        IStructuredDocument structuredDocument = fDocument;
        IStructuredDocumentRegion fnode =
                structuredDocument.getRegionAtCharacterOffset(offStart);
        ...
        for (int i = 0; i < veclen; i++) {
            holdResults.add(returnPrunedStyle(offStart2,
                    length2,
                    (StyleRange) vecTempResults.get(i)));
        }
        return retval;
    }
    ...
    /**
	 */
    public boolean prepareRegions2(IStructuredDocumentRegion fnode,
            ITypedRegion currentRegion, int offStart, int length,
        ...
        try {
                ...
                if (cacheSize > 0) {
                    ...
                    if (nd.offCachedStylesRegion != offNode) {
                        ...
                        for (int sidx = 0; sidx < cacheSize; sidx++) {
                            StyleRange sr =
                                    (StyleRange) nd.cachedStyles.get(sidx);
                            sr.start += delta;
                        }
                    }
                    ...
                    StyleRange srLastCached =
                            (StyleRange) nd.cachedStyles.get(cacheSize - 1);
                    if ((((StyleRange) nd.cachedStyles.get(0)).start <= offStart)
                                ...
                                if (srOne.start > offStart) {
                                    if (iihigh == ii) {
                                        if (Debug.jsDebugSyntaxColoring)
                                            Logger
                                                    .log(Logger.ERROR,
                                                            "someone corrupted the js style range cache. Infinite loop aborted."); //$NON-NLS-1$
                                        return false;
                                    } //$NON-NLS-1$
                                    iihigh = ii;
            ...
            }
            {
                ...
                ILexer lexer2 =
                        getParseStartPoint(fnode, null, offStart2, startHere);
                ...
                while (tkThis.getLPOffset() < (offsMax + 200)) {
                    ...
                    if (offsTkThisEnd <= offsReq) {
                        ...
                        attr = XPathColorPreferences.taDefault;
                        if (tkThis instanceof TStringLiteral) {
                            attr = XPathColorPreferences.taStringLit;
                            ...
                            attr = XPathColorPreferences.taUnfComment;
                            ...
                            attr = XPathColorPreferences.taKeyword;
                        }
                        ...
                        if (Debug.jsDebugSyntaxColoring
                            ...
                            throw new RuntimeException(
                                    "assert failed: we didn't use unique constants for highligh colors"); //$NON-NLS-1$
                        }
            ...
            Logger.log(Logger.WARNING_DEBUG,
                    "Exception in JavaScript prepareRegions()", exc); //$NON-NLS-1$
     ...
     * this is if it extends the range of styles and doesn't want to check if he
     * is extending it beyond the end of the acceptable region. (Details
     * omitted.) In this case, he gets a second copy so that he can decide which
     * copy gets extended.
     */
    public static StyleRange returnPrunedStyleWithEndDup(int offRegStart,
            int lenReg, StyleRange srIn) {
