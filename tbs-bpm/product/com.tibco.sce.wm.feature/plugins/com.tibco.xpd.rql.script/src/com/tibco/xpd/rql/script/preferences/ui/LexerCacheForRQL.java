...
import com.tibco.xpd.script.sourceviewer.internal.contentassist.JSContentElementConstants;
import com.tibco.xpd.script.sourceviewer.internal.contentassist.JSContentElementImpl;
 ...
 */
public class LexerCacheForRQL implements ITokenCache {
	...
	public static LexerCacheForRQL getCache(Object key, String strInit) {
		...
		if (val == null) {
			...
			LexerCacheForRQL lc = new LexerCacheForRQL();
			lc.notifyChange(strInit, 0, 0, 0);//$NON-NLS-1$
			htCaches.put(key, lc);
			return lc;
		}
		else {
			return (LexerCacheForRQL) val;
		}
	}
	...
	public TextRange notifyChange(String strNew, int posChange, int lenOld, int lenNew) {
		...
		if (lenNew < 0)
			throw new RuntimeException(Messages.internal_error_1); //$NON-NLS-1$
		//if (lenOld<0) throw new RuntimeException( "internal error: can't pass a negative lenNew to LCFJS.notifyChange" );
		if (strNew == null)
			throw new RuntimeException(Messages.internal_error_2); //$NON-NLS-1$
