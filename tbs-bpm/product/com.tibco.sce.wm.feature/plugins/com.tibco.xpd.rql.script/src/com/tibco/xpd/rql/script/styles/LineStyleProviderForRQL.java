...
public class LineStyleProviderForRQL implements LineStyleProvider, IDocumentListener {
    public static HashSet<String> entityKeywords = new HashSet<String>();
    
    public static HashSet<String> attributeKeywords = new HashSet<String>();
    
    public static HashSet<String> operatorKeywords = new HashSet<String>();
	...
	static {
        entityKeywords.addAll(RQLUtils.getEntityList());
        attributeKeywords.addAll(RQLUtils.getEntityAttributeList());
        operatorKeywords.addAll(RQLUtils.getOperatorCombinatorList());
	}
	...
	public LineStyleProviderForRQL() {
		super();
	}
	...
	protected ILexer getParseStartPoint(IStructuredDocumentRegion fnode, ITextRegion region, int offStart, boolean startHere) {
		...
		if (nd.pcParseCache == null) {
			...
			if (fnode != null) {
				...
				// pcParseCache = LexerCacheForJavaScript.getCache(targetNode,
				// strOldNodeValue);
				nd.pcParseCache = LexerCacheForRQL.getCache(fnode, nd.strOldNodeValue);
				nd.pcParseCache.notifyChange(nd.strOldNodeValue, -1, 0, 0);
			}
			else {
				// pcParseCache = LexerCacheForJavaScript.getCache(targetNode,
				// strOldNodeValue==null ? "":strOldNodeValue ); //$NON-NLS-1$
				nd.pcParseCache = LexerCacheForRQL.getCache(fnode, nd.strOldNodeValue == null ? "" : nd.strOldNodeValue); //$NON-NLS-1$
			}
		}
	...
	public void documentChanged(DocumentEvent event) {
		...
		while (keys.hasMoreElements()) {
			...
			if (event.fOffset <= idxNodeStart && idxChgEnd >= idxNodeEnd) {
				// fnode deleted
			    LexerCacheForRQL.release(fnode);
	...
	public void nodeChanged(IStructuredDocumentRegion fnode, int idxInsertPos, int iReplacedLen, String strInserted) {
		...
		try {
			if (fnode != null) {
			    LexerCacheForRQL lx = LexerCacheForRQL.getCache(fnode, ""); //$NON-NLS-1$
				if (nd.pcParseCache == null) {
					// do nothing
				}
				else {
