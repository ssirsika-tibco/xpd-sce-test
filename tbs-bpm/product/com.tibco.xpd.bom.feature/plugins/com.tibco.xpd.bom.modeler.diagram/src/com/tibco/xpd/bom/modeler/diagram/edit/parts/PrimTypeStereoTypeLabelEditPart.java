...
public class PrimTypeStereoTypeLabelEditPart extends
     ...
     * @generated
     */
    public PrimTypeStereoTypeLabelEditPart(View view) {
        super(view);
    }
    ...
    public void setLabel(WrappingLabel figure) {
        ...
        // Add the Label if needed on initialization
        PrimitiveType primT = (PrimitiveType) resolveSemanticElement();
        if (!primT.getAppliedStereotypes().isEmpty()
                && !FirstClassProfileManager.getInstance()
                        .isFirstClassProfileApplied(primT.getModel())) {

            // EList<Stereotype> lstStereos = primT.getAppliedStereotypes();
            //
            //            if (!(lstStereos.size() == 1 && ("RestrictedType".equals(lstStereos //$NON-NLS-1$
            // .get(0).getName())))) {
            // PrimitiveTypeEditPart pep = (PrimitiveTypeEditPart) getParent();
            // pep.getCustFigure().rebuildFigureWithStereo(true);
            // }

            if (BOMEditPartUtils.shouldRebuildFigureToShowStereotype(primT)) {
                PrimitiveTypeEditPart pep = (PrimitiveTypeEditPart) getParent();
                pep.getCustFigure().rebuildFigureWithStereo(true);
            }
