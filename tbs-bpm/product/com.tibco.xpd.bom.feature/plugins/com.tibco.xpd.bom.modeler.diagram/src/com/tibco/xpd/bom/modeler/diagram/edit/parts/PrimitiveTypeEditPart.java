...
public class PrimitiveTypeEditPart extends ShapeNodeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof PrimitiveTypeNameEditPart) {
			((PrimitiveTypeNameEditPart) childEditPart).setLabel(getPrimaryShape().getFigurePrimitiveTypeNameLabel());
			return true;
		}
		if (childEditPart instanceof PrimTypeSuperTypeNameLabelEditPart) {
			((PrimTypeSuperTypeNameLabelEditPart) childEditPart)
					.setLabel(getPrimaryShape().getFigurePrimTypeSuperTypeNameLabel());
			return true;
		}
		if (childEditPart instanceof PrimTypeStereoTypeLabelEditPart) {
			((PrimTypeStereoTypeLabelEditPart) childEditPart)
					.setLabel(getPrimaryShape().getFigurePrimTypeStereoTypeLabelFigure());
			return true;
		}
		return false;
	}
					...
					if (notification.getNewValue() != null && notification.getNewValue() instanceof Edge) {
						...
						if (edge.getElement() instanceof Generalization) {
							...
							if (gen.eContainer() == resolveSemanticElement()) {
								...
								getCustFigure().rebuildFigureWithSuperType(false);
	...
	protected void handleNotificationEvent(Notification notification) {
		...
		if (notification.getNotifier() instanceof Generalization && notification.getEventType() == Notification.SET) {
			...
			if (notification.getOldValue() instanceof PrimitiveType
				...
				PrimitiveType oldSuperType = (PrimitiveType) notification.getOldValue();
				PrimitiveType newSuperType = (PrimitiveType) notification.getNewValue();
				...
				if (oldSuperType.eIsProxy() && !newSuperType.eIsProxy()) {
					...
					for (Object object : childEPs) {
						if (object instanceof PrimTypeSuperTypeNameLabelEditPart) {
							PrimTypeSuperTypeNameLabelEditPart ep = (PrimTypeSuperTypeNameLabelEditPart) object;
							ep.refresh();
							break;
						}
					}
				...
				&& (notification.getEventType() == Notification.ADD
						&& notification.getNewValue() instanceof Generalization)
				|| (notification.getEventType() == Notification.REMOVE
						&& notification.getOldValue() instanceof Generalization)) {
			// Base type changed, refresh the diagram
			PrimitiveTypeFigure custFigure2 = this.getCustFigure();

			Generalization value = (Generalization) (notification.getNewValue() == null ? notification.getOldValue()
					: notification.getNewValue());

			if (elem.eResource() == value.getGeneral().eResource() && notification.getEventType() == Notification.ADD) {
				// If we're adding the base type make sure we add any relevant generalisation
				// arrows
				custFigure2.rebuildFigureWithSuperType(false);
				refreshGeneralizations();
			} else if (elem.eResource() == value.getGeneral().eResource()
					&& notification.getEventType() == Notification.REMOVE) {
		...
		if (elem != null) {
			...
			if (elem instanceof PrimitiveType) {
				...
				if (!primType.getGeneralizations().isEmpty()) {
					Generalization generalization = primType.getGeneralizations().get(0);
		...
		if (superT instanceof PrimitiveType) {
			...
			if (superType.eResource() != thisType.eResource()) {
				...
				if (text.indexOf("BomPrimitiveTypes::") != -1) { //$NON-NLS-1$
					...
					ArrayList<PrimitiveType> foundNames = new ArrayList<PrimitiveType>();

					UML2ModelUtil.searchPackageForPrimType(modType, superType.getName(), foundNames);
	...
	public void refreshVisuals() {
		...
		GradientFigureUtil.updateFigureGradient(getPrimaryView(), custFigure,
				FigureUtilities.RGBToInteger(
						PreferenceConverter.getColor(store, IBOMPreferenceConstants.PREF_PRIMTYPE_GRAD_COLOR1)),
				FigureUtilities.RGBToInteger(
						PreferenceConverter.getColor(store, IBOMPreferenceConstants.PREF_PRIMTYPE_GRAD_COLOR2)));
	...
	public Object getPreferredValue(EStructuralFeature feature) {
		Object result = GradientFigureUtil.getPreferedPrimitiveTypeGradientValue(this, feature);
		if (result == null) {
