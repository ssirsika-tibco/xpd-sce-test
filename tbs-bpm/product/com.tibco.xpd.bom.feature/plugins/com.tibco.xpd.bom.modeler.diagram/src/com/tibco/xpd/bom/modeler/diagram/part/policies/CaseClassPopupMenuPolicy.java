...
/**
 * Checks if the right click menu on a Class should contain the convert to Case
 * Class entry
 * 
 */
public class CaseClassPopupMenuPolicy implements IPopupMenuContributionPolicy {
        ...
        if (selection instanceof StructuredSelection) {
            ...
            for (Object object : elements.toList()) {
                if ((object != null) && (object instanceof ClassEditPart)) {
                    ...
                    // is already a case class, in which case we do not want to
                    // show the menu item
                    if ((toMutateClass == null)
                            || GlobalDataProfileManager.getInstance()
                                    .isCase(toMutateClass)) {
                        return false;
                    }
                }
            }
