...
public abstract class AbstractAppearanceSection extends
    ...
    protected SelectedColor showColorPalette(Button button, int currentColor) {
        final ColorPalettePopup popup = new ColorPalettePopup(button
                .getParent().getShell(), IDialogConstants.BUTTON_BAR_HEIGHT);
        popup.setPreviousColor(currentColor);

        Rectangle r = button.getBounds();
        Point location = button.getParent().toDisplay(r.x, r.y);
        Point loc = new Point(location.x + r.width, location.y + r.height
    ...
    protected void disposeImage(Image image) {
        ...
        if (image.equals(OMDiagramUIPropertiesImages
