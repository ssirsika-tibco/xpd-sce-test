    ...
    private boolean isCustomExecutable(Request request) {
        if (request instanceof EditCommandRequestWrapper) {
            ...
            if (editCommandRequest instanceof DestroyElementRequest) {
                ...
                // Command should only be executable for Organizations and
                // OrgUnitRelationships that span Organizations.
                if (elementToDestroy instanceof OrgUnitRelationship) {
                    OrgUnitRelationship rel = (OrgUnitRelationship) elementToDestroy;

                    OrgUnit to = rel.getTo();
                    OrgUnit from = rel.getFrom();

                    if (to.eContainer() == from.eContainer()) {
                        return false;
                    }
                } else if (!(elementToDestroy instanceof Organization)) {
                    return false;
                }
            }
        }
