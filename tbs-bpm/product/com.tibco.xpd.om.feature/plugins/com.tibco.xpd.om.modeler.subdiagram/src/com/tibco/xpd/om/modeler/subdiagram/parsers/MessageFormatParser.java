package com.tibco.xpd.om.modeler.subdiagram.parsers;
...
public class MessageFormatParser extends AbstractParser {
	...
	public String getPrintString(IAdaptable adapter, int flags) {
		...
		if (element instanceof Position) {
			...
			text = getViewProcessor().format(getValues(element),
			...
			if (position.getFeature() != null) {
				...
				EList<ResourceAssociation> resourceAssociation = position
				...
				for (ResourceAssociation resAssoc : resourceAssociation) {
					...
					// And append Human resource type (a way of displaying the
					// person assigned to a positio for example.
					if (resourceType.isHumanResourceType()) {
						text = text + " (" + resource.getDisplayName() + ")"; //$NON-NLS-1$ //$NON-NLS-1$
					}

				}

			}
		} else if (element instanceof OrgUnitRelationship) {
			OrgUnitRelationship rel = (OrgUnitRelationship) element;
			text = getViewProcessor().format(getValues(element),
					new StringBuffer(), new FieldPosition(0)).toString();

			if (rel.getType() != null) {
				text = text + " <" + rel.getType().getDisplayName() //$NON-NLS-1$
						+ ">"; //$NON-NLS-1$
			}

		} else {
			text = getPrintStringGen(adapter, flags);
		}
	 ...
	 * @generated
	 */
	public String getPrintStringGen(IAdaptable adapter, int flags) {
		EObject element = (EObject) adapter.getAdapter(EObject.class);
	...
	public IParserEditStatus isValidEditString(IAdaptable adapter,
		...
		if (values == null) {
			return new ParserEditStatus(
					OrganizationModelSubDiagramEditorPlugin.ID,
	...
	protected ICommand getModificationCommand(EObject element,
		...
		/*
		 * If an OrgUnit's name has changed then make sure all connection names
		 * are updated
		 */
		if (element instanceof OrgUnit
				&& (feature == OMPackage.eINSTANCE.getNamedElement_Name() || feature == OMPackage.eINSTANCE
						.getNamedElement_DisplayName())) {

			IEditingDomainItemProvider provider = (IEditingDomainItemProvider) XpdResourcesPlugin
					.getDefault().getAdapterFactory().adapt(element,
			...
			if (provider != null && editingDomain != null) {
				Command cmd = provider.createCommand(element, editingDomain,
						SetCommand.class, new CommandParameter(element,
								feature, value));

				if (cmd != null) {
					CompositeCommand compositeCmd = new CompositeCommand(cmd
							.getLabel());
					compositeCmd
							.add(new EMFCommandOperation(editingDomain, cmd));
					return compositeCmd;
				}
			}
		}

		return super.getModificationCommand(element, feature, value);
	}
}
