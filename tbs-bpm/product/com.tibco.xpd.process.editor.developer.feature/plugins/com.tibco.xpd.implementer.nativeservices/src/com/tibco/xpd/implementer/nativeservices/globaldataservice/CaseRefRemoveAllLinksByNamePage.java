...
/**
 * Properties section for remove all link by association name operation of the Global Data Task.
 * 
 * @author jarciuch
 */
public class CaseRefRemoveAllLinksByNamePage extends AbstractOperationPage<CaseReferenceOperationsType> {

    private ContentAssistText unlinkAssociationPath;

    /**
     * Remove link(s) to all case object(s) operation properties section of the Global Data Task.
     * 
     * @param section
     */
    public CaseRefRemoveAllLinksByNamePage(GlobalDataTaskServiceSection section) {
        super(Messages.CaseRefRemoveAllLinksByNamePage_label, section);
    }
    ...
    public Control createPage(Composite parent, XpdFormToolkit toolkit) {
        ...
        Control c = createWrappedDescriptionText(toolkit,
                unlinkCtrlsContainer,
                Messages.CaseRefRemoveAllLinksByNamePage_desc);
        c.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

        Composite innerContainer = toolkit.createComposite(unlinkCtrlsContainer);
        ...
        Label lb = toolkit.createLabel(innerContainer, Messages.GlobalDataTaskServiceSection_caseClassAssoc_label);
        lb.setLayoutData(new GridData());

        unlinkAssociationPath = createContentAssistText(innerContainer,
                toolkit,
                new CaseReferenceOperationAssociationProposalProvider());

        unlinkAssociationPath.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
    ...
    public void update(CaseReferenceOperationsType opType) {
        RemoveAllLinksByNameType removeAllLinksByName = opType.getRemoveAllLinksByName();
        if (removeAllLinksByName != null) {
            updateText(unlinkAssociationPath.getText(), removeAllLinksByName.getAssociationName());
        } else {
     ...
     */
    @Override
    public Command getCommand(EditingDomain editingDomain, CaseReferenceOperationsType opType, Object control) {
        Command cmd = null;
        if (control == unlinkAssociationPath.getText()) {
            /*
             * update the case association in the unlink operation of a case reference operation
             */
            if (opType != null && opType.getRemoveAllLinksByName() != null) {
                String text = ((Text) control).getText().trim();
                cmd = SetCommand.create(editingDomain,
                        opType.getRemoveAllLinksByName(),
                        XpdExtensionPackage.eINSTANCE.getRemoveAllLinksByNameType_AssociationName(),
                        text);
            }
        }
        return cmd;
