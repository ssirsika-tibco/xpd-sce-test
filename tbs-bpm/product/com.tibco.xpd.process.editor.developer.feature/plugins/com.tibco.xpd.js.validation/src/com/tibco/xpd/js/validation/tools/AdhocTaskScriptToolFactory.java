/*
 * Copyright (c) TIBCO Software Inc 2004, 2014. All rights reserved.
 */

package com.tibco.xpd.js.validation.tools;

import com.tibco.xpd.processeditor.xpdl2.util.ProcessScriptUtil;
import com.tibco.xpd.validation.provider.IPreProcessor;
import com.tibco.xpd.validation.provider.IValidationScope;
import com.tibco.xpd.xpdl2.Activity;

/**
 * Provides with the Script Tool Factory to deal with Adhoc Precondition
 * Scripts.
 * 
 * @author kthombar
 * @since 15-Aug-2014
 */
public class AdhocTaskScriptToolFactory extends JavaScriptToolFactory {

    @Override
    public IPreProcessor createPreProcessor(IValidationScope scope, Object input) {
        IPreProcessor processor = null;
        if (input instanceof Activity) {
            Activity activity = (Activity) input;
            boolean b =
                    ProcessScriptUtil
                            .isAdhocPreconditionTaskWithScriptType(activity,
                                    getScriptGrammar());
            if (b) {
                processor = new AdhocTaskScriptTool(activity);
            }
        }
        return processor;
    }

    @Override
    public Class<? extends IPreProcessor> getToolClass() {
        return AdhocTaskScriptTool.class;
    }
}