 ...
 * 
 */
public class CompletedScriptTaskScriptToolFactory extends JavaScriptToolFactory {

	public IPreProcessor createPreProcessor(IValidationScope scope, Object input) {
		IPreProcessor processor = null;
		if (input instanceof Activity) {
			Activity activity = (Activity) input;
			boolean b = ProcessScriptUtil.isActivityWithCompletedScriptType(activity,
					getScriptGrammar());
			if (b) {
				processor = new CompletedScriptTaskScriptTool(activity);
			}
		}
		return processor;
	}
	...
	public Class<? extends IPreProcessor> getToolClass() {
		return CompletedScriptTaskScriptTool.class;
	}
