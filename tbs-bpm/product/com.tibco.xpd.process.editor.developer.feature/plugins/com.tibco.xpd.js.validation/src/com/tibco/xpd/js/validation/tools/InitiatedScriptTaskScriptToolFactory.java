 ...
 * 
 */
public class InitiatedScriptTaskScriptToolFactory extends JavaScriptToolFactory {

	public IPreProcessor createPreProcessor(IValidationScope scope, Object input) {
		IPreProcessor processor = null;
		if (input instanceof Activity) {
			Activity activity = (Activity) input;
			boolean b = ProcessScriptUtil.isActivityWithInitiatedScriptType(activity,
					getScriptGrammar());
			if (b) {
				processor = new InitiatedScriptTaskScriptTool(activity);
			}
		}
		return processor;
	}
	...
	public Class<? extends IPreProcessor> getToolClass() {
		return InitiatedScriptTaskScriptTool.class;
	}
