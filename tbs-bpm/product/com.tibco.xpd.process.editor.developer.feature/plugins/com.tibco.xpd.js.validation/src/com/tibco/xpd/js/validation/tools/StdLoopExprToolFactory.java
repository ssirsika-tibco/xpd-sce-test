 ...
 * 
 */
public class StdLoopExprToolFactory extends JavaScriptToolFactory {

    public IPreProcessor createPreProcessor(IValidationScope scope, Object input) {
        IPreProcessor processor = null;
        if (input instanceof Activity) {
            Activity activity = (Activity) input;
            boolean b = ProcessScriptUtil.isActivityStdLoopExprHasGrammar(activity,
                    getScriptGrammar());
            if (b) {
                processor = new StdLoopExprTool(activity);
            }
        }
        return processor;
    }
