 ...
 * 
 */
public class TaskScriptToolFactory extends JavaScriptToolFactory {

	public IPreProcessor createPreProcessor(IValidationScope scope, Object input) {
		IPreProcessor processor = null;
		if (input instanceof Activity) {
			Activity activity = (Activity) input;
			boolean b = ProcessScriptUtil.isScriptTaskWithScriptType(activity,
					getScriptGrammar());
			if (b) {
				processor = new TaskScriptTool(activity);
			}
		}
		return processor;
	}
