...
public class UserTaskOpenScriptToolFactory extends JavaScriptToolFactory {

	public IPreProcessor createPreProcessor(IValidationScope scope, Object input) {
		IPreProcessor processor = null;
		if (input instanceof Activity) {
			Activity activity = (Activity) input;
			boolean b = ProcessScriptUtil.isUserTaskWithOpenScriptType(activity,
					getScriptGrammar());
			if (b) {
				processor = new UserTaskOpenScriptTool(activity);
			}
		}
		return processor;
	}
	...
	public Class<? extends IPreProcessor> getToolClass() {
		return UserTaskOpenScriptTool.class;
	}
