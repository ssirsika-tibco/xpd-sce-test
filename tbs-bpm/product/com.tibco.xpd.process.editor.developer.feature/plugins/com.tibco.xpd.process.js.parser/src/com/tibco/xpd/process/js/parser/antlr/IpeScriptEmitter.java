// $ANTLR 2.7.7 (20060906): "IpeScript.tree.emitter.g" -> "IpeScriptEmitter.java"$
...
public class IpeScriptEmitter extends antlr.TreeParser       implements IpeScriptEmitterTokenTypes
 {
...
public IpeScriptEmitter() {
	...
	public final void stat(AST _t) throws RecognitionException {
		...
		try {      // for error handling
			...
			case LITERAL_while:
			{
				...
				expression(_t);
				_t = _retTree;
				conditionalExpression = false;out(" ");indent();insertGeneratedStatement(true,false);undent();
				stat(_t);
				_t = _retTree;
				_t = __t53;
				_t = _t.getNextSibling();
			...
			{
				AST __t54 = _t;
				doWhileStat = _t==ASTNULL ? null :(AST)_t;
				match(_t,LITERAL_do);
				_t = _t.getFirstChild();
	...
	public final void builtInType(AST _t) throws RecognitionException {
		...
		AST builtInType_AST_in = (_t == ASTNULL) ? null : (AST)_t;
			String corrKeyWord = TranslateUtil.getCorrespondingKeyWord(builtInType_AST_in.getText());
			if(corrKeyWord!=null){
				out(corrKeyWord+" ");
			}else{
	...
	public final void expression(AST _t) throws RecognitionException {
		...
		try {      // for error handling
			...
			_t = _t.getFirstChild();
							...
							AST assignAST = expression_AST_in.getFirstChild();
							TranslateUtil.handleBooleanValue(expression_AST_in);
							TranslateUtil.handleStringLiteral(expression_AST_in);
							TranslateUtil.handleDateTimeDotAST(expression_AST_in,this);
							if(assignAST!=null)
							{
	...
	public final void ifElseStat(AST _t) throws RecognitionException {
		...
		try {      // for error handling
			...
			condExpr = _t==ASTNULL ? null : (AST)_t;
			expression(_t);
			_t = _retTree;
			...
			trueBlock = _t==ASTNULL ? null : (AST)_t;
			slist(_t);
			_t = _retTree;
			{
			if (_t==null) _t=ASTNULL;
	...
	public final void expr(AST _t) throws RecognitionException {
		...
		try {      // for error handling
			...
			{
				AST __t88 = _t;
				postAST = _t==ASTNULL ? null :(AST)_t;
				match(_t,POST_INC);
				_t = _t.getFirstChild();
			...
			{
				AST __t89 = _t;
				postDecAST = _t==ASTNULL ? null :(AST)_t;
				match(_t,POST_DEC);
				_t = _t.getFirstChild();
	...
	public final void primaryExpression(AST _t) throws RecognitionException {
		...
		try {      // for error handling
			...
			case LITERAL_null:
			{
				...
				out(TranslateUtil.getCorrespondingKeyWord(primaryExpression_AST_in.getText()));
				break;
			}
			case TYPE:
			{
	...
	public final void constant(AST _t) throws RecognitionException {
		...
		if (constant_AST_in.getType() == STRING_LITERAL) { 
			out(constant_AST_in.getText().replace("\\\"", "\"\""));
		} else {
