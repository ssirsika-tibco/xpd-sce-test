...
/**
 * 
 * Action on the Package editor to add a new Process Interface.
 * 
 * @author rsomayaj
 * @since 3.3 (11 Apr 2010)
 */
public class AddProcessIfcAction extends Action {
     ...
     * @param defaultSel
     */
    public AddProcessIfcAction(Object defaultSel) {
        super(
                Messages.PackageEditorFormPage_CreateProcIfc_label,
                Xpdl2ProcessEditorPlugin
                        .getImageDescriptor(ProcessEditorConstants.IMG_PROCESS_IFC_NEW));
    ...
    @Override
    public void run() {
        NewProcessInterfaceWizard newProcessInterfaceWizard =
                new NewProcessInterfaceWizard();
        newProcessInterfaceWizard.init(PlatformUI.getWorkbench(),
                new StructuredSelection(defaultSelection));

        WizardDialog dialog =
                new WizardDialog(Display.getCurrent().getActiveShell(),
                        newProcessInterfaceWizard);
        dialog.open();
