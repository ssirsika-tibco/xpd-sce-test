...
/**
 * The service task mappings generation isn't one that is updated on each
 ...
 * 
 */
public class UpdateServiceTaskMappings extends UpdateJavaScriptMappingsCommand {
     ...
     * @param obj
     */
    public UpdateServiceTaskMappings(EditingDomain editingDomain,
            Activity serviceTaskActivity) {
        super(editingDomain, serviceTaskActivity);
    ...
    @Override
    public void execute() {
        appendAndExecuteRemoveScriptInformationCommand(serviceTaskActivity);

        /*
         * Update the mappings for the given activity type.
         */
        TaskType taskType = TaskObjectUtil.getTaskType(serviceTaskActivity);
        if (TaskType.SERVICE_LITERAL.equals(taskType)) {
            TaskService taskService =
                    ((Task) serviceTaskActivity.getImplementation())
                            .getTaskService();
            updateTaskServiceMappings(serviceTaskActivity, taskService);
        }
    }
    ...
    private void updateTaskServiceMappings(Activity activity,
        ...
        if (taskService.getMessageIn() != null) {
            clearedMessageIn = getClearedMessage(taskService.getMessageIn());
            this.appendAndExecute(SetCommand.create(getEditingDomain(),
                    taskService,
                    Xpdl2Package.eINSTANCE.getTaskService_MessageIn(),
                    clearedMessageIn));
        }
        Message clearedMessageOut = null;
        if (taskService.getMessageOut() != null) {
            clearedMessageOut = getClearedMessage(taskService.getMessageOut());
            this.appendAndExecute(SetCommand.create(getEditingDomain(),
                    taskService,
        ...
        List<DataMapping> outMappings = new ArrayList<DataMapping>();
            ...
            if (!ProcessInterfaceUtil.isImplicitAssociationDisabled(activity)) {
                ...
                for (DataField df : dataFields) {
                    ...
                    DataMapping outMapping =
                            createOutMapping(activity.getName(),
                                    df.getName(),
                                    ModeType.OUT_LITERAL,
                                    DirectionType.OUT_LITERAL);
                    if (null != outMapping) {
                        dataMappings.add(outMapping);
                    }
                }
            }
    ...
    protected List<DataMapping> createDataMappings(NamedElement namedElement,
        ...
        DataMapping inMapping =
                createInMapping(serviceTaskActivity.getName(),
                        dataMappingName,
                        modeType,
                        DirectionType.IN_LITERAL);
        if (inMapping != null) {
        ...
        DataMapping outMapping =
                createOutMapping(serviceTaskActivity.getName(),
                        dataMappingName,
                        modeType,
                        DirectionType.OUT_LITERAL);
        if (outMapping != null) {
