 ...
 * @author nwilson
 */
public class PageflowToProcessCompositor implements SingleMappingCompositor {
     ...
     *            The target.
     */
    public PageflowToProcessCompositor(Activity activity, String target) {
    ...
    public Collection<Object> getSourceItems(boolean isInput) {
        ...
        Process process = TaskObjectUtil.getUserTaskPageflowProcess(activity);

        if (process != null) {
            for (String name : getSourceItemNames()) {
                ConceptPath path =
                        SubProcUtil.resolveParameter(process, direction, name);
                items.add(path);
            }
        }
        return items;
