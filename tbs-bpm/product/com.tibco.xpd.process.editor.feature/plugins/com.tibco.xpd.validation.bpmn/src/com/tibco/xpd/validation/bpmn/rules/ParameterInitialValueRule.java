...
/**
 * Validation to check values inserted for Initial Values on Formal Parameters.
 * 
 * @author rsomayaj
 */
public class ParameterInitialValueRule extends DataTypeInitialValueRule {
    
    protected String DUPLICATE_VALUE = "bpmn.parameterInitialValueDuplicateValue"; //$NON-NLS-1$

    /**
     * Initialise message string for this rule
     */
    public ParameterInitialValueRule(){
    ...
    /**
     * Gets the list of data from the parameter object so we can later
    ...
    protected String[] initialValueSetup(ProcessRelevantData pd){
        ...
        if (formalParameter.
                  getOtherElement(XpdExtensionPackage.eINSTANCE.getDocumentRoot_InitialValues().getName()) != null){
            List<String> tempList = ((InitialValues) formalParameter
    ...
    public void validate(Object o) {
        ...
        ArrayList<ProcessRelevantData> formalParameters =
            new ArrayList<ProcessRelevantData>(process.getFormalParameters());
        parameterOnlyValidation(formalParameters);
        
        super.validateFieldsOrParams(formalParameters);        
    }
    
    protected void parameterOnlyValidation(ArrayList<ProcessRelevantData> formalParameters){
