...
public class CaseDocumentQueryExpressionImpl extends EObjectImpl implements CaseDocumentQueryExpression
{
	...
	public void setQueryExpressionJoinType(QueryExpressionJoinType newQueryExpressionJoinType)
	{
		...
		queryExpressionJoinType = newQueryExpressionJoinType == null ? QUERY_EXPRESSION_JOIN_TYPE_EDEFAULT
	...
	public void setOperator(CMISQueryOperator newOperator)
	{
		...
		operatorESet = true;
		if (eNotificationRequired()) eNotify(new ENotificationImpl(this, Notification.SET,
				XpdExtensionPackage.CASE_DOCUMENT_QUERY_EXPRESSION__OPERATOR, oldOperator, operator, !oldOperatorESet));
	}
	...
	public void setCmisDocumentPropertySelected(boolean newCmisDocumentPropertySelected)
	{
		...
		if (eNotificationRequired()) eNotify(new ENotificationImpl(this, Notification.SET,
				...
				oldCmisDocumentPropertySelected, cmisDocumentPropertySelected, !oldCmisDocumentPropertySelectedESet));
	}
	...
	public String toString()
	{
		...
		if (queryExpressionJoinTypeESet) result.append(queryExpressionJoinType);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(", openBracketCount: "); //$NON-NLS-1$
		if (openBracketCountESet) result.append(openBracketCount);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(", cmisProperty: "); //$NON-NLS-1$
		if (cmisPropertyESet) result.append(cmisProperty);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(", operator: "); //$NON-NLS-1$
		if (operatorESet) result.append(operator);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(", processDataField: "); //$NON-NLS-1$
		if (processDataFieldESet) result.append(processDataField);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(", closeBracketCount: "); //$NON-NLS-1$
		if (closeBracketCountESet) result.append(closeBracketCount);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(", cmisDocumentPropertySelected: "); //$NON-NLS-1$
		if (cmisDocumentPropertySelectedESet) result.append(cmisDocumentPropertySelected);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(')');
