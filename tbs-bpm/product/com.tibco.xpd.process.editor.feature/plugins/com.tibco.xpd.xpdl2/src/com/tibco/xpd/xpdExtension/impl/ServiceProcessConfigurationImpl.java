...
public class ServiceProcessConfigurationImpl extends EObjectImpl implements ServiceProcessConfiguration
{
	...
	public void setDeployToProcessRuntime(boolean newDeployToProcessRuntime)
	{
		...
		if (eNotificationRequired()) eNotify(new ENotificationImpl(this, Notification.SET,
				XpdExtensionPackage.SERVICE_PROCESS_CONFIGURATION__DEPLOY_TO_PROCESS_RUNTIME, oldDeployToProcessRuntime,
				deployToProcessRuntime, !oldDeployToProcessRuntimeESet));
	}
	...
	public void unsetDeployToProcessRuntime()
	{
		...
		if (eNotificationRequired()) eNotify(new ENotificationImpl(this, Notification.UNSET,
				XpdExtensionPackage.SERVICE_PROCESS_CONFIGURATION__DEPLOY_TO_PROCESS_RUNTIME, oldDeployToProcessRuntime,
				DEPLOY_TO_PROCESS_RUNTIME_EDEFAULT, oldDeployToProcessRuntimeESet));
	}
	...
	public String toString()
	{
		...
		if (deployToProcessRuntimeESet) result.append(deployToProcessRuntime);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(", deployToPageflowRuntime: "); //$NON-NLS-1$
		if (deployToPageflowRuntimeESet) result.append(deployToPageflowRuntime);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(')');
