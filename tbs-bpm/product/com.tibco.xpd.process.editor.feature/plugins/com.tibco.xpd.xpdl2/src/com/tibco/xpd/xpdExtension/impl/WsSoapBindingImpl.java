...
public class WsSoapBindingImpl extends WsBindingImpl implements WsSoapBinding
{
	...
	public void setSoapVersion(String newSoapVersion)
	{
		...
		if (eNotificationRequired()) eNotify(new ENotificationImpl(this, Notification.SET,
				XpdExtensionPackage.WS_SOAP_BINDING__SOAP_VERSION, oldSoapVersion, soapVersion, !oldSoapVersionESet));
	}
	...
	public String toString()
	{
		...
		if (bindingStyleESet) result.append(bindingStyle);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(", soapVersion: "); //$NON-NLS-1$
		if (soapVersionESet) result.append(soapVersion);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(')');
