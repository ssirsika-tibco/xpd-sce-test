...
public class WsSoapJmsOutboundBindingImpl extends WsSoapBindingImpl implements WsSoapJmsOutboundBinding
{
	...
	public void setOutboundSecurity(WsSoapSecurity newOutboundSecurity)
	{
		if (newOutboundSecurity != outboundSecurity)
		{
			...
			if (outboundSecurity != null) msgs = ((InternalEObject) outboundSecurity).eInverseRemove(this,
					EOPPOSITE_FEATURE_BASE - XpdExtensionPackage.WS_SOAP_JMS_OUTBOUND_BINDING__OUTBOUND_SECURITY, null,
					msgs);
			if (newOutboundSecurity != null) msgs = ((InternalEObject) newOutboundSecurity).eInverseAdd(this,
					EOPPOSITE_FEATURE_BASE - XpdExtensionPackage.WS_SOAP_JMS_OUTBOUND_BINDING__OUTBOUND_SECURITY, null,
					msgs);
	...
	public void setPriority(int newPriority)
	{
		...
		priorityESet = true;
		if (eNotificationRequired()) eNotify(new ENotificationImpl(this, Notification.SET,
				XpdExtensionPackage.WS_SOAP_JMS_OUTBOUND_BINDING__PRIORITY, oldPriority, priority, !oldPriorityESet));
	}
	...
	public String toString()
	{
		...
		if (deliveryModeESet) result.append(deliveryMode);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(", priority: "); //$NON-NLS-1$
		if (priorityESet) result.append(priority);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(", messageExpiration: "); //$NON-NLS-1$
		if (messageExpirationESet) result.append(messageExpiration);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(", invocationTimeout: "); //$NON-NLS-1$
		if (invocationTimeoutESet) result.append(invocationTimeout);
		else
			result.append("<unset>"); //$NON-NLS-1$
		result.append(')');
