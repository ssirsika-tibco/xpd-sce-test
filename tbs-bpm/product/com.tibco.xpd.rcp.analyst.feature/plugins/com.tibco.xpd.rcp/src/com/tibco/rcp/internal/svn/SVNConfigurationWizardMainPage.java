...
/**
 * copy of
 * org.tigris.subversion.subclipse.ui.wizards.ConfigurationWizardMainPage with
 * some minor modifications.
 * 
 * @author kupadhya
 * @since 21 Jan 2013
 */
public class SVNConfigurationWizardMainPage extends SVNWizardPage {
     ...
     */
    public SVNConfigurationWizardMainPage(String pageName, String title,
            ImageDescriptor titleImage) {
        super(pageName, title, titleImage);
        showCredentials =
                SVNProviderPlugin.getPlugin().getSVNClientManager()
                        .getSvnClientInterface().equals(COMMANDLINE_CLIENT);
    }
    ...
    public void createControl(Composite parent) {
        ...
        PlatformUI
                .getWorkbench()
                .getHelpSystem()
                .setHelp(composite, IHelpContextIds.SHARING_NEW_REPOSITORY_PAGE);
        ...
        Group g =
                createGroup(composite,
                        Messages.SVNConfigurationWizardMainPage_Location_1);

        // repository Url
        createLabel(g, Messages.SVNConfigurationWizardMainPage_url);
        ...
        if (showCredentials) {
            g =
                    createGroup(composite,
                            Messages.SVNConfigurationWizardMainPage_Authentication_2);

            // User name
            createLabel(g, Messages.SVNConfigurationWizardMainPage_userName);
            userCombo = createEditableCombo(g);
            userCombo.addListener(SWT.Selection, listener);
            userCombo.addListener(SWT.Modify, listener);

            // Password
            createLabel(g, Messages.SVNConfigurationWizardMainPage_password);
    ...
    private void validateFields() {
        ...
        try {
            ...
            setErrorMessage(String
                    .format(Messages.SVNConfigurationWizardMainPage_invalidUrl,
                            e.getMessage()));
            setPageComplete(false);
            return;
        }
    ...
    public boolean canFlipToNextPage() {
        if (getWizard() instanceof SVNCheckOutWizard) {
