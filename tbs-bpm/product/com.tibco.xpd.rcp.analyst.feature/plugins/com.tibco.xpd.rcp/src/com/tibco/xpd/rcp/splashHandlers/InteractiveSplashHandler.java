...
public class InteractiveSplashHandler extends AbstractSplashHandler {
	...
	private void handleButtonOKWidgetSelected() {
		...
		if ((username.length() > 0) &&
			...
			MessageDialog.openError(
					getSplash(),
					"Authentication Failed",  //NON-NLS-1 //$NON-NLS-1$
					"A username and password must be specified to login.");  //NON-NLS-1 //$NON-NLS-1$
		}
	}
	...
	private void createUIButtonCancel() {
		...
		fButtonCancel.setText("Cancel"); //NON-NLS-1 //$NON-NLS-1$
	...
	private void createUIButtonOK() {
		...
		fButtonOK.setText("OK"); //NON-NLS-1 //$NON-NLS-1$
		// Configure layout data
		GridData data = new GridData(SWT.NONE, SWT.NONE, false, false);
		data.widthHint = F_BUTTON_WIDTH_HINT;
		data.verticalIndent = 10;
	...
	private void createUILabelPassword() {
		// Create the label
		Label label = new Label(fCompositeLogin, SWT.NONE);
		label.setText("&Password:"); //NON-NLS-1 //$NON-NLS-1$
		// Configure layout data
	...
	private void createUILabelUserName() {
		// Create the label
		Label label = new Label(fCompositeLogin, SWT.NONE);
		label.setText("&User Name:"); //NON-NLS-1 //$NON-NLS-1$
		// Configure layout data
		GridData data = new GridData();
		data.horizontalIndent = F_LABEL_HORIZONTAL_INDENT;
		label.setLayoutData(data);		
