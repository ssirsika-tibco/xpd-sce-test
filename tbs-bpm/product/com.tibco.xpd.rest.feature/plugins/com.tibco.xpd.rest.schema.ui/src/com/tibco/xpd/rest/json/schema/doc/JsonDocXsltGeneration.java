...
import com.tibco.xpd.resources.ui.util.LocaleUtils;
...
/**
 * This class is responsible for the generation of documentation for the JSON
 * model(both index.html and xslt doc)
 * 
 * @author kthombar
 * @since June 04, 2015
 */
public class JsonDocXsltGeneration extends AbstractXSLTDocGen {

    private static final String XSLT = "/xslts/json2html.xsl"; //$NON-NLS-1$
    ...
    public Map<String, String> getXsltParameters() {
        ...
        if (currentDocGenInfo.getSource() instanceof IFile) {
            IFile sourceFile = (IFile) currentDocGenInfo.getSource();
            /*
             * passing the json file name as nowhere in json file do we store
             * the file name, hence there is no way that the xsl file can
             * determine the file name.
             */
            map.put("jsonFileName", sourceFile.getName()); //$NON-NLS-1$
            IPath fullPath = sourceFile.getFullPath();
            map.put("jsdFilePath", fullPath.toString()); //$NON-NLS-1$
    ...
    protected String getGeneratorRootFolderName() {
        ...
        return Messages.JsonDocXsltGeneration_JsonSchemaFolder_title;
    }
    ...
    protected List<IDocGenModelInfo> createDocGenModelInfo(IResource resource) {
        ...
        if (resource != null) {
            ...
            if (workingCopy != null) {
                ...
                String jSONImagePath = getGeneratorRootFolderName() + "/" //$NON-NLS-1$
                        + ResourceCopier.IMAGES_FOLDER + "/" //$NON-NLS-1$
                        + ResourceCopier.JSON_FILE_ICON;

                populateDocGenModelInfo(docGenXPDLModelInfo,
                        null,
                        modelName,
                        modelName,
                        "-", // JSON has no description//$NON-NLS-1$
                        changeDate,
                        "-", // JSON has no Author//$NON-NLS-1$
                        null,
                        getLink(),
                        null,
                        null,
                        jSONImagePath,
                        Messages.JsonDocXsltGeneration_JsonSchemaModelContainer_title,
                        "700", // Setting priority =700 //$NON-NLS-1$
                        jSONImagePath);
