...
/**
 * This class is responsible for the generation of documentation for the REST
 * model(both index.html and xslt doc)
 * 
 * @author kthombar
 * @since June 04, 2015
 */
public class RestServiceDocXsltGeneration extends AbstractXSLTDocGen {

    private static final String XSLT = "/xslts/rest2html.xsl"; //$NON-NLS-1$
    ...
    public Map<String, String> getXsltParameters() {
        ...
        if (currentDocGenInfo.getSource() instanceof IFile) {
             ...
             * the file name.
             */
            map.put("restFileName", sourceFile.getName()); //$NON-NLS-1$
            IPath fullPath = sourceFile.getFullPath();
            map.put("restFilePath", fullPath.toString()); //$NON-NLS-1$
    ...
    protected String getGeneratorRootFolderName() {
        ...
        return Messages.RestServiceDocXsltGeneration_RestServiceDocFolder_name;
    }
    ...
    protected List<IDocGenModelInfo> createDocGenModelInfo(IResource resource) {
        ...
        if (resource != null) {
            ...
            if (workingCopy != null) {
                ...
                String generatorRootFolderName = getGeneratorRootFolderName();

                String restServiceImagePath = generatorRootFolderName + "/" //$NON-NLS-1$
                        + ResourceCopier.IMAGES_FOLDER + "/" //$NON-NLS-1$
                        + ResourceCopier.REST_SERVICE_ICON;
                ...
                populateDocGenModelInfo(docGenXPDLModelInfo,
                        ...
                        null,
                        null,
                        generatorRootFolderName + "/" //$NON-NLS-1$
                                + ResourceCopier.IMAGES_FOLDER + "/" //$NON-NLS-1$
                                + ResourceCopier.RSD_FILE_ICON,
                        Messages.RestServiceDocXsltGeneration_RestServiceModelContainer_title,
                        "600", // Setting priority =600 //$NON-NLS-1$
                        restServiceImagePath);
