 ...
 * associated case refs as argument when multiplicity is > 1 and returns void.
 * also provides java doc for this method in the content assist
 * 
 * @author bharge
 * @since 30 Oct 2013
 */
public class CaseRefLinkAllMethod extends AbstractJsMethod {
     ...
     * 
     */
    private final String LINK = "link"; //$NON-NLS-1$
     ...
     * @param multiValued
     */
    public CaseRefLinkAllMethod(Class caseClass, Property property,
            String caseRefName, boolean multiValued) {
        ...
        this.methodName =
                LINK + changeCaseInitialChar(property.getName(), true);
        this.comment =
                getLinkJavaDoc(caseClass,
                        property,
                        multiValued,
                        caseRefName,
                        methodName);
     ...
     * @return String
     */
    private String getLinkJavaDoc(Class caseClass, Property property,
            boolean isMultiple, String caseRefName, String methodName) {

        String comment =
                Messages.ScriptContentAssist_javadoc_comment_definition_and_usage
                        + Messages.GlobalDataGenerator_comment_ref_link_attributes_description
                        + "\n\n"
                        + Messages.ScriptContentAssist_javadoc_comment_syntax
                        + Messages.GlobalDataGenerator_comment_ref_link_attributes_syntax
                        + "\n\n"
                        + Messages.ScriptContentAssist_javadoc_comment_param
                        + Messages.GlobalDataGenerator_comment_ref_link_attributes_param
                        + "\n\n"
                        + Messages.ScriptContentAssist_javadoc_comment_return
                        + Messages.GlobalDataGenerator_comment_ref_link_attributes_return;
        comment =
