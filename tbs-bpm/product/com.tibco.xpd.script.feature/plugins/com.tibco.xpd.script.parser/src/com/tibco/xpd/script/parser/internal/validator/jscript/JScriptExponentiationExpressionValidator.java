...
/**
 * 
 * Expression validator for the validation of exponentiation expressions
 * 
 * @author mtorres
 */
public class JScriptExponentiationExpressionValidator extends
        ...
        if (null != expression) {
            ...
            if (expr instanceof AST) {
                ...
                case JScriptTokenTypes.BXOR_ASSIGN:
                    ...
                    // Check if the types for lhs and rhs of the exponential
                    // expressions are correct
                    IScriptRelevantData evaluateExponentationOperator =
                            evaluateExponentationOperator((Token) token,
                                    lhsDataType,
                                    rhsDataType);
                    if (evaluateExponentationOperator != null) {
                        returnDataType = evaluateExponentationOperator;
                    }
                    break;
                default:
                    break;
     ...
     * @return
     */
    private IScriptRelevantData evaluateExponentationOperator(Token token,
            IScriptRelevantData lhsDataType, IScriptRelevantData rhsDataType) {
        IScriptRelevantData dataType =
                getExponentationOperatorType(lhsDataType, rhsDataType);
        ...
        } else if (!isValidExponentationOperator(lhsDataType, rhsDataType)) {
            String errorMessage =
                    Messages.ExpressionValidator_ExponentationOperationBetween;
            List<String> additionalAttributes = new ArrayList<String>();
    ...
    protected IScriptRelevantData getExponentationOperatorType(
            IScriptRelevantData lhsDataType, IScriptRelevantData rhsDataType) {
        IScriptRelevantData dataType =
                createScriptRelevantData(JsConsts.UNDEFINED_DATA_TYPE,
                        JsConsts.UNDEFINED_DATA_TYPE,
        ...
        if (lhsDataType != null && lhsDataType.getType() != null
            ...
            dataType =
                    createScriptRelevantData(lhsStrType,
                            lhsStrType,
                            false,
                            createGenericContext(lhsDataType, isGenericContextArray(lhsDataType, lhsDataType)), null);
        }
        return dataType;
    }

    protected boolean isValidExponentationOperator(
            IScriptRelevantData lhsDataType, IScriptRelevantData rhsDataType) {
        if (lhsDataType != null && lhsDataType.getType() != null
                && rhsDataType != null && rhsDataType.getType() != null) {
            ...
            Map<String, Set<String>> compatibleExponentationOperatorTypesMap =
                    null;
            ...
            if (dataTypeMapper instanceof IJScriptDataTypeMapper) {
                ...
                compatibleExponentationOperatorTypesMap =
                        jsDataTypeMapper
                                .getCompatibleExponentiationOperatorTypesMap();
                compatibleExponentationOperatorTypesMap =
                    jsDataTypeMapper
                            .convertSpecificMapToGeneric(compatibleExponentationOperatorTypesMap);
            }
            if (compatibleExponentationOperatorTypesMap != null) {
                Set<String> compatibleExponentationOperatorSet =
                        compatibleExponentationOperatorTypesMap.get(lhsStrType);
                if (compatibleExponentationOperatorSet != null
                        && compatibleExponentationOperatorSet
                                .contains(rhsTypeStr)) {
                    return true;
                }
            }
