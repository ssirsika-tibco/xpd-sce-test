...
import com.tibco.xpd.scripteditors.internal.javascript.styles.LineStyleProviderForJavaScript;

/**
 * The JavaScript Lexer does not conform to the top level
 * IStructuredDocumentRegion model nor its RegionParser interface. Since the
 * StyledTextColorPicker depends on having a usable parser, wrap the Lexer
 * accordingly.
 */
...
public class TokenParser implements RegionParser {
    ...
    /**
     * Convert the JavaScript Lexer token list into one large
     * IStructuredDocumentRegion mapping each token and type to a region and
     * context.
     */
    public IStructuredDocumentRegion getDocumentRegions() {
        ...
        while (token != null && !(token instanceof EOF)) {
            ...
            if (token instanceof TStringLiteral) {
                ...
                type = IStyleConstantsJavaScript.UNFINISHED_COMMENT;
            } else if (token instanceof TErrorChar) { // like a backslash
                                                      // character
                type = IStyleConstantsJavaScript.UNFINISHED_COMMENT;
