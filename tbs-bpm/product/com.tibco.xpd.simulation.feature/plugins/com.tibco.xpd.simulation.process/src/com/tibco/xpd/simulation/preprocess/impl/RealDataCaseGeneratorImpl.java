...
public class RealDataCaseGeneratorImpl implements CaseGenerator {
     ...
     *            number of cases.
     */
    public RealDataCaseGeneratorImpl(Process process, Cases cases,
            int noOfCases) {
        this(process, cases, noOfCases, 997L);
    }
    ...
    public RealDataCaseGeneratorImpl(Process process, Cases cases,
        ...
        if (noOfCases <= 0) {
            this.noOfCases = fileNoOfCases;
        } else {
            this.noOfCases = Math.min(noOfCases, fileNoOfCases);
        }
        bufferNoOfCases = Math.min(this.noOfCases, MAX_NO_OF_CASES);
    ...
    private void generateParams(Process process) {
        List wsParams = SimulationXpdlUtils
    ...
    public Iterator iterator() {
        return new Iterator() {
            ...
            public Object next() {
                Case fileCase = (Case) cases.getCase().get(index);
                Date fileCaseDate = DateUtil.parseDate(fileCase
                        .getStartTime().toString());
                
                List fileCaseParams = fileCase.getParamter();
                CaseParameter[] caseParams = new CaseParameter[noOfParams];
                for (int i = 0; i < caseParams.length; i++) {

                    String paramName = paramNames[i];
                    Object paramValue = data[i].get(index % bufferNoOfCases);
                    //
                    // for first found parameter from file with mathing name
                    // replace value.
                    for (Iterator iter = fileCaseParams.iterator(); iter
                            .hasNext();) {
                        Paramter fileCaseParam = (Paramter) iter.next();
                        if (paramName.equals(fileCaseParam.getName())) {
                            paramValue = fileCaseParam.getValue();
                            break;
                        }
                    }
                    caseParams[i] = new CaseParameterImpl(paramName, paramValue);
                }
                ...
                index++;
                return new CaseImpl(caseId, caseParams, fileCaseDate);
    ...
    private static double getParameterWeightSum(ParameterDistribution param) {
        ...
        for (Iterator iter = param.getEnumerationValue().iterator(); iter
                .hasNext();) {
            sum += ((EnumerationValueType) iter.next()).getWeightingFactor();
        }
        return sum;
    }

    /**
     * Returns the number of values for specified enumeration value based on
     * percentage and noOfCases.
     * 
     * @param enumeration
     *            enumeration value for which there will be estimated number of
     *            values.
     * @param percentageSum
     *            sum of all enumeration percentages for parameter.
     * @param noOfCases
     *            number of cases.
     * @return number of values.
     */
    private static long getNumberOfValuesForEnumeration(
            EnumerationValueType enumeration, double percentageSum,
            int noOfCases) {
