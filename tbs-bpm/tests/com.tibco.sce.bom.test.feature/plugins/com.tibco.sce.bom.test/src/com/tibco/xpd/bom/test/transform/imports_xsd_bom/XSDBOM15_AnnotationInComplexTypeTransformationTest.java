...
public class XSDBOM15_AnnotationInComplexTypeTransformationTest extends
    ...
    protected String MODEL_FILE =
            "XSDBOM15_AnnotationInComplexTypeTransformationTest.xsd";
    ...
    private void checkBOMElements(Model model) throws Exception {
        ...
        PackageableElement elem = packagedElements.get(0);
        assertTrue("BOM element is not of type Class", elem instanceof Class); //$NON-NLS-1$

        assertEquals("Name of class", "concept3", elem.getName()); //$NON-NLS-1$

        // Class should have 1 attribute of type integer
        Class tempCls = (Class) elem;
        EList<Property> ownedAttributes = tempCls.getOwnedAttributes();

        assertEquals("Number of attributes in class", 1, ownedAttributes.size()); //$NON-NLS-1$

        Property property = ownedAttributes.get(0);
        assertEquals("Name of attribute", "id", property.getName()); //$NON-NLS-1$
        ...
        assertEquals("Property Type Integer", integerType, property.getType()); //$NON-NLS-1$
        TransformUtil.assertPropertyMultiplicity(property, 0, 1);

        EList<Comment> ownedComments = tempCls.getOwnedComments();
        assertEquals("Number of comments in class", 1, ownedComments.size()); //$NON-NLS-1$

        Comment comment = ownedComments.get(0);
        String trim = comment.getBody().trim();
        assertEquals("Check comment content",
                "One of many concept examples here",
                trim);
