...
/**
 * 
 * Junit test for protecting issue fixed in XPD-4907. Resolves conflicts if a
 * complex anonymous type has same name as the top level element that its child
 * element references. example: Complex Anonymous type name -> SimpleRef ; its
 * child element refers a top level element which has name -> SimpleRefType
 * 
 * @author kthombar
 * @since 27-Jun-2013
 */
public class XSDBOM32_ComplexAnonymousTypeReferenceTest extends
        TransformationTestRoundtrip {

    protected String TEST_FILE_NAME =
            "XSDBOM32_ComplexAnonymousTypeReferenceTest";
    ...
    protected String MODEL_FILE =
            "XSDBOM32_ComplexAnonymousTypeReferenceTest.xsd";
    ...
    private void checkBOMElements(Model model) throws Exception {
        ...
        // And its name to be "SimpleType1"
        Assert.assertEquals("Unexpected name for Enumeration.",
                "submitTokenType",
                allEnumerations.get(0).getName());
        ...
        for (Class eachClass : allClazzes) {
            if (eachClass.getName().equals("SubmitTaskReqType")) {
                ...
                assertEquals("Unexpected Attribute Type",
                        "SubmitTaskReqTypeType",
                        eachClass.getOwnedAttributes().get(0).getType()
                                .getName());
            } else if (eachClass.getName().equals("SubmitTaskReqTypeType")) {
                ...
                for (Property attr : ownedAttr) {
                    if (attr.getName().equals("submitToken")) {
                        // Check is attribute data type is correct
                        assertEquals("Unexpected Attribute Type",
                                "submitTokenType",
