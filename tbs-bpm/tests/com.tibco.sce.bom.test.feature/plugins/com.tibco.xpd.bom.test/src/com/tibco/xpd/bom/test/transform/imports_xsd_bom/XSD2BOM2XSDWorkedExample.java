...
/**
 * Template derived from current test suuite.
 * 
 * @since 12 Jun 2013
 */
@SuppressWarnings("nls")
public class XSD2BOM2XSDWorkedExample extends TransformationTestRoundtrip {

    protected String TEST_FILE_NAME = "XSD2BOM2XSDWorkedExample";

    protected String PLATFORM_EXAMPLE_FILES_BASE =
            "platform:/plugin/com.tibco.xpd.bom.test/test-resources/xsd";

    // The name of the XML Schema that is being tested
    protected String MODEL_FILE = "WorkedExample.xsd";
    ...
    private void checkBOMElements(Model model) throws Exception {
        ...
        Assert.assertEquals("Unexpected name for PrimitiveType.",
                "SimpleType1",
                allPrimTypes.get(0).getName());

        // Inspect the Class
        // We expect there to be only one
        Assert.assertEquals("Unexpected number of Class.", 1, allClazzes.size());

        // And its name to be "ComplexType1"
        Assert.assertEquals("Unexpected name for PrimitiveType.",
                "ComplexType1",
                allClazzes.get(0).getName());

        // Now lets check if there are 3 Property attributes for this class
        Class cl = allClazzes.get(0);
        ArrayList<Property> allPropertys = new ArrayList<Property>();
        EList<Property> ownedAttributes = cl.getOwnedAttributes();

        for (Property prop : ownedAttributes) {
            allPropertys.add(prop);
        }

        Assert.assertEquals("Unexpected number of Propertys.",
                3,
                allPropertys.size());
