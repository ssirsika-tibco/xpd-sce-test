...
public class XSDBOM07_ComplexContentExtensionTransformationTest extends
    ...
    protected String MODEL_FILE =
            "XSDBOM07_ComplexContentExtensionTransformationTest.xsd";
    ...
    private void checkBOMElements(Model model) throws Exception {
        // For this test we should have 2 classes named extFile2 and
        // file2
        ...
        // Superclass checks
        EList<Property> supeOwnedAttributes = superClass.getOwnedAttributes();
        assertEquals("Number of attributes in attGroup Class", 2, supeOwnedAttributes.size()); //$NON-NLS-1$
        TransformUtil.assertAttributeInClass(supeOwnedAttributes,
                "description",
        ...
        // Subclass checks
        EList<Property> subOwnedAttributes = subClass.getOwnedAttributes();
        assertEquals("Number of attributes in Class", 1, subOwnedAttributes.size()); //$NON-NLS-1$
        TransformUtil.assertAttributeInClass(subOwnedAttributes,
                "moredetails",
                PrimitivesUtil.BOM_PRIMITIVE_TEXT_NAME);

        EList<Generalization> generalizations = subClass.getGeneralizations();
        assertEquals("Number of generalizations in extFile2 Class", 1, generalizations.size()); //$NON-NLS-1$

        Generalization generalization = generalizations.get(0);
        assertEquals("Generalizes file2", generalization.getGeneral(), superClass); //$NON-NLS-1$
