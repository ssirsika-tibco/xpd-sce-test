...
/**
 * 
 * Junit test for protecting XPD-4938. Checks and protect the generation of
 * .bom2xsd if an Element[e] and a Simple type have same name and a child of
 * complex type is referencing the Element[e].
 * 
 * @author kthombar
 * @since 04-Jul-2013
 */
public class XSDBOM33_SimpleTypeAndElementHavingSameNameTransformationTest
        extends TransformationTestRoundtrip {

    protected String TEST_FILE_NAME =
            "XSDBOM33_SimpleTypeAndElementHavingSameNameTransformationTest";
    ...
    protected String MODEL_FILE =
            "XSDBOM33_SimpleTypeAndElementHavingSameNameTransformationTest.xsd";
    ...
    private void checkBOMElements(Model model) throws Exception {
        ...
        for (Class eachClass : allClazzes) {
            if (eachClass.getName().equals("noteType")) {
                ...
                for (Property prop : ownedAttributes) {
                    if (prop.getName().equals("to")) {
                        Assert.assertEquals("Unexpected Type.", "toType", prop
                                .getType().getName());
                    }
                }

            } else if (eachClass.getName().equals("nomeType")) {
                Assert.assertEquals("Unexpected number of Attributes.",
                        3,
                        eachClass.getOwnedAttributes().size());
                EList<Property> ownedAttributes =
                ...
                for (Property prop : ownedAttributes) {
                    if (prop.getName().equals("nome")) {
                        Assert.assertEquals("Unexpected Type.", "nome1", prop
                                .getType().getName());
                    }
                }

            } else if (eachClass.getName().equals("Nome")) {
                Assert.assertEquals("Unexpected number of Attributes.",
                        3,
                        eachClass.getOwnedAttributes().size());
                EList<Property> ownedAttributes =
