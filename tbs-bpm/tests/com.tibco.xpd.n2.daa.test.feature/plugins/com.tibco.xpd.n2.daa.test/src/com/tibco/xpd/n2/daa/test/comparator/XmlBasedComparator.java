/*
 * Copyright (c) TIBCO Software Inc 2004, 2009. All rights reserved.
 */

package com.tibco.xpd.n2.daa.test.comparator;

import java.io.IOException;
import java.io.InputStream;

import org.custommonkey.xmlunit.DetailedDiff;
import org.custommonkey.xmlunit.Diff;
import org.custommonkey.xmlunit.DifferenceListener;
import org.custommonkey.xmlunit.ElementQualifier;
import org.eclipse.core.runtime.IStatus;
import org.eclipse.core.runtime.Status;
import org.xml.sax.InputSource;
import org.xml.sax.SAXException;

import com.tibco.xpd.core.test.util.FileComparator;
import com.tibco.xpd.n2.daa.test.Activator;

/**
 * @author kupadhya
 * 
 */
public class XmlBasedComparator extends FileComparator {
    protected static final String NAME = "name"; //$NON-NLS-1$

    @Override
    public IStatus compareContents(InputStream goldFile,
            InputStream generatedFile, String fileName) {

        IStatus toReturn = Status.OK_STATUS;
        InputSource goldIS = new InputSource(goldFile);
        InputSource generatedIS = new InputSource(generatedFile);
        try {

            Diff myDiff = new Diff(goldIS, generatedIS);
            DetailedDiff detailedDiff = new DetailedDiff(myDiff);
            ElementQualifier elementQualifier = getElementQualifier();
            if (elementQualifier != null) {

                detailedDiff.overrideElementQualifier(elementQualifier);
            }
            DifferenceListener differenceListener = getDifferenceListener();
            if (differenceListener != null) {

                detailedDiff.overrideDifferenceListener(differenceListener);
            }
            boolean identical = detailedDiff.identical();
            if (identical) {

                toReturn =
                        new Status(IStatus.OK, Activator.PLUGIN_ID,
                                "Generated file contents is identical to gold file " //$NON-NLS-1$
                                        + fileName);
            } else {

                toReturn =
                        new Status(IStatus.ERROR, Activator.PLUGIN_ID,
                                "Generated file contents IS NOT IDENTICAL gold file contents " //$NON-NLS-1$
                                        + fileName);
            }
        } catch (SAXException saxEx) {

            toReturn =
                    new Status(IStatus.ERROR, Activator.PLUGIN_ID,
                            "SAXException occurred " + saxEx.getMessage()); //$NON-NLS-1$
        } catch (IOException ioEx) {

            toReturn =
                    new Status(IStatus.ERROR, Activator.PLUGIN_ID,
                            "IOException occurred " + ioEx.getMessage()); //$NON-NLS-1$
        }
        return toReturn;
    }

    protected DifferenceListener getDifferenceListener() {

        return null;
    }

    protected ElementQualifier getElementQualifier() {

        return null;
    }
}
