...
public class PeSharedResourceContributorTest extends TestCase {
    ...
    public void testProjectWithoutContributions1() throws Exception {
        ...
        try {
            ...
            RascAppSummary appSummary = mock(RascAppSummary.class);
            when(appSummary.getVersion()).thenReturn(version);
            when(appSummary.getReferencedProjects())
                    .thenReturn(Collections.emptyList());
            when(Boolean.valueOf(appSummary.hasAssetType(anyString())))
    ...
    public void testBpmReferences() throws Exception {
        ...
        try {
            ...
            RascDependency bomDependency = mock(RascDependency.class);
            when(bomDependency.getInternalName())
                    .thenReturn("com.test.bom.module");
            when(bomDependency.getDependencyRange())
                    .thenReturn(new VersionRange("(1.2.3,2]"));
            when(Boolean.valueOf(bomDependency.hasAssetType(
                    com.tibco.xpd.bom.resources.ui.Activator.BOM_ASSET_ID)))
                            .thenReturn(Boolean.TRUE);

            RascDependency nonBomDependency = mock(RascDependency.class);
            when(nonBomDependency.getInternalName())
                    .thenReturn("com.test.nonbom.module");
            when(nonBomDependency.getDependencyRange())
                    .thenReturn(new VersionRange("(1.2.4,2]"));
            when(Boolean.valueOf(nonBomDependency.hasAssetType(anyString())))
                    .thenReturn(Boolean.TRUE);
            ...
            when(Boolean.valueOf(appSummary.hasAssetType(anyString())))
                    .thenReturn(Boolean.FALSE);
            when(appSummary.getReferencedProjects())
                    .thenReturn(Arrays.asList(nonBomDependency, bomDependency));
            ...
            String[] expectedArtifacts =
                    { "SimpleProc-Process", "data-dependencies" };
            List<WriterContent> artifacts = writer.getArtifacts();
            assertEquals(expectedArtifacts.length, artifacts.size());
            for (WriterContent artifact : artifacts) {
                boolean found = false;
